...
fn dfs(g: &Graph, v: usize, p: usize, s: &[char], ord: &mut Vec<(usize, usize)>) -> bool {
    g.children(v, p)
        .map(|u| {
            let c = dfs(g, u, v, s, ord);
            if c {
                ord.push((u, v));
            } else {
                ord.push((v, u));
...
fn main() {
    ...
    let mut ord = vec![];
    if !dfs(&g, 0, n, &s, &mut ord) {
        println!("-1");
        return;
    }

    let g1 = Graph::from_edges_directed(n, ord);

    let mut in_degs = (0..n).map(|i| g1.in_edges[i].len()).collect::<Vec<_>>();
    let mut q = (0..n)
