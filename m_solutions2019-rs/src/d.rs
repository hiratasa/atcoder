...
fn dfs(g: &Graph, v: usize, p: usize, idx: usize, c: &[usize], ans: &mut [usize]) -> usize {
    ans[v] = c[idx];

    g.children(v, p)
        .fold(idx + 1, |next, u| dfs(g, u, v, next, c, ans))
}
...
fn main() {
    ...
    let c = c.citer().sorted().rev().collect::<Vec<_>>();

    let s = c.citer().skip(1).sum::<usize>();

    let g = Graph::from_edges1_undirected(n, ab);

    let mut ans = vec![0; n];
    dfs(&g, 0, n, 0, &c, &mut ans);
