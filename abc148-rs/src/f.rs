...
fn dfs(
    ...
    height[v] = 1 + g
        .children(v, p)
        .map(|u| dfs(g, u, v, d + 1, depth, height))
        .max()
        .unwrap_or(0);
...
fn main() {
    let (n, u, v) = read_tuple!(usize, usize, usize);
    ...
    dfs(&g, v, n, 0, &mut depth, &mut height);

    let path = find_path(&g, v, n, u).unwrap();

    let ans = path
        .citer()
        .filter(|&x| depth[u] - depth[x] < depth[x])
        .map(|x| (depth[u] - depth[x]) + (height[x] - 1) + (depth[x] - (depth[u] - depth[x]) - 1))
        .max()
        .unwrap();
