...
fn main() {
    ...
    let g = detail::Graph::from_edges_directed(
        n,
        (0..n).flat_map(|i| {
            s[i].citer()
                .positions(|d| d == 1)
    ...
    let dp0 = (0..n).fold(vvec![0; usize::MAX; n], |mut dp, i| {
        g.out_edges[i].citer().for_each(|e| {
            let j = e.to;
            dp[j] = min(dp[j], dp[i].saturating_add(1));
        });

        dp
    });
    let dp1 = (0..n).rev().fold(vec![usize::MAX; n], |mut dp, i| {
        if i == n - 1 {
            dp[i] = 0;
        }
        g.in_edges[i].citer().for_each(|e| {
            let j = e.from;
            dp[j] = min(dp[j], dp[i].saturating_add(1));
        });
    ...
    println!(
        ...
        (1..n - 1)
            .map(|i| {
                (i.saturating_sub(10)..i)
                    .filter_map(|j| {
                        g.adjs(j)
                            .filter(|&k| k > i)
                            .map(|k| dp0[j].saturating_add(1).saturating_add(dp1[k]))
