...
fn main() {
    ...
    let g = Graph::from_edges1_directed(n, xy);

    let in_degs = (0..n).map(|i| g.in_edges[i].len()).collect::<Vec<_>>();
    let zeros = in_degs.citer().positions(|d| d == 0).collect::<Vec<_>>();

    let orders = (0..n)
        .scan((in_degs, zeros), |(in_degs, zeros), _| {
            if zeros.len() != 1 {
                None
            } else {
                let v = zeros.pop().unwrap();
    ...
    if orders.len() == n {
        let ans = orders
            .citer()
            .enumerate()
            .fold(vec![0; n], |mut ans, (i, j)| {
                ans[j] = i + 1;
                ans
            });
        println!("Yes");
        println!("{}", ans.citer().map(|x| x).join(" "));
    } else {
        println!("No");
    }
}
