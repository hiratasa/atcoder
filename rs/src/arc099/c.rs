...
fn check_bipartite(
    ...
    once(usize::MAX)
        .chain(g.adjs(v).sorted())
        .chain(once(n))
        .tuple_windows()
        .flat_map(|(i, j)| i.wrapping_add(1)..j)
        .filter(|&u| u != v)
        .map(|u| {
            if visited[u] {
                ...
                colors[u] = !colors[v];
                check_bipartite(g, u, visited, colors)
            }
        })
        .try_fold((1, 0), |(s0, s1), sc| sc.map(|(s2, s3)| (s0 + s3, s1 + s2)))
}
...
fn solve(g: &Graph) -> Option<usize> {
    ...
    let t = (0..n)
        .scan((vec![false; n], vec![false; n]), |(visited, colors), v| {
            if !visited[v] {
                Some(check_bipartite(&g, v, visited, colors))
            } else {
                Some(Some((0, 0)))
            }
        })
    ...
    let n0 = t.citer().map(|(x, y)| min(x, y)).sum::<usize>();

    let dp = t
        .citer()
        .fold(bitset!(n - 2 * n0 + 1, 1), |mut bs, (x, y)| {
            bs.shl_or(max(x, y) - min(x, y));
            bs
        });
    ...
    (n0..=n - n0)
        .filter(|&i| dp[i - n0])
        .map(|i| (i, n - i))
        .map(|(k, l)| k * k.saturating_sub(1) / 2 + l * l.saturating_sub(1) / 2)
        .min()
}
