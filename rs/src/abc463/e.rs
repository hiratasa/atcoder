fn main() {
    ...
    let g = Graph::from_edges_directed(
        ...
        uvt.iter()
            .copied()
            .flat_map(|(u, v, t)| [(u, v, t), (v, u, t)])
            .chain(x.iter().copied().enumerate().map(|(i, x)| (i, n, x + y)))
            .chain(x.iter().copied().enumerate().map(|(i, x)| (n, i, x))),
    );
    ...
    while let Some((Reverse(d), v)) = q.pop() {
        ...
        for &e in &g.out_edges[v] {
            if d + e.label < dists[e.to] {
                q.push((Reverse(d + e.label), e.to));
                dists[e.to] = d + e.label;
            }
        }
    }

    println!("{}", (1..n).map(|i| dists[i]).join(" "));
}
