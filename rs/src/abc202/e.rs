...
fn dfs(
    ...
    let map = g.out_edges[v]
        .citer()
        .map(|edge| dfs(g, edge.to, depth + 1, queries, ans))
        .fold(
            once((depth, 1)).collect::<FxHashMap<_, _>>(),
            |mut map0, mut map1| {
    ...
    for (i_query, d) in queries[v].citer() {
        ans[i_query] = map.get(&d).copied().unwrap_or(0);
    }
...
fn main() {
    ...
    let q: usize = read();
    let ud = read_vec(q, || read_tuple!(usize, usize));

    let queries = ud
        .citer()
        .enumerate()
        .fold(vec![vec![]; n], |mut queries, (i, (u, d))| {
            queries[u - 1].push((i, d));
            queries
        });

    let mut ans = vec![0; q];
    dfs(&g, 0, 0, &queries, &mut ans);
