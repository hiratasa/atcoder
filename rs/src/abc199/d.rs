...
fn dfs(g: &Graph, v: usize, colors: &mut [Option<usize>], parents: &mut [Option<usize>]) -> usize {
    let color0 = colors[v].unwrap();
    ...
    g.out_edges[v]
        ...
        .map(|edge| {
            if let Some(color) = colors[edge.to] {
                ...
                parents[edge.to] = Some(v);

                colors[edge.to] = Some((color0 + 1) % 3);
                let y0 = dfs(g, edge.to, colors, parents);

                colors[edge.to] = Some((color0 + 2) % 3);
                let y1 = dfs(g, edge.to, colors, parents);
...
fn main() {
    ...
    let (_, ans) = (0..n).fold((vec![None; n], 1), |(mut parents, ans), i| {
        if parents[i].is_none() {
            let mut colors = vec![None; n];
            colors[i] = Some(0);
            let t = dfs(&g, i, &mut colors, &mut parents);
            (parents, ans * 3 * t)
        } else {
            (parents, ans)
        }
