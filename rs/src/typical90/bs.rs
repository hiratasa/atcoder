...
fn solve(
    ...
    for i in (0..m).rev() {
        q.swap(i, m - 1);
        let u = q.pop().unwrap();
        if !solve(g, u, k, degs, q, ans, current) {
            return false;
        }
        q.push(u);
        q.swap(i, m - 1);
...
fn main() {
    ...
    let mut q = (0..n).filter(|&i| degs[i] == 0).collect::<Vec<_>>();
    let mut ans = vec![];

    let l = q.len();
    for i in 0..l {
        let u = q.remove(i);
        if !solve(&g, u, k, &mut degs, &mut q, &mut ans, &mut vec![]) {
            println!("-1");
            return;
        }
        q.insert(i, u);
    ...
    for a in ans {
        println!("{}", a.citer().map(|x| x + 1).join(" "));
    }
}
