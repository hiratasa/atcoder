...
fn dfs(g: &Graph, v: usize, visited: &mut [bool], ans: &mut Vec<usize>) {
    if visited[v] {
        return;
    }
    visited[v] = true;
    for &edge in &g.out_edges[v] {
        dfs(g, edge.to, visited, ans);
    }
...
fn main() {
    ...
    for i in 0..n {
        ...
        if ab[i].0 == i + 1 || ab[i].1 == i + 1 {
            dfs(&g, i, &mut visited, &mut ans);
        }
    }

    if visited.citer().all(|v| v) {
        ans.citer().map(|i| i + 1).for_each(|i| println!("{}", i));
    } else {
        println!("-1");
    }
}
