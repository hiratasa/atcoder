...
fn dfs(
    ...
    g.out_edges[v]
        ...
        .fold(idxs[v], |last_idx, edge| {
            idxs[edge.to] = last_idx + 1;
            depth[edge.to] = depth[v] + 1;
            parent[edge.to] = v;

            dfs(g, edge.to, v, idxs, depth, parent)
        })
}

fn lca(v: usize, u: usize, depth: &[usize], parents: &[Vec<usize>]) -> usize {
    let (v, u) = if depth[u] < depth[v] { (u, v) } else { (v, u) };

    let u = parents
        .iter()
        .rev()
        .fold(u, |u, ps| if depth[ps[u]] >= depth[v] { ps[u] } else { u });

    if v == u {
        return v;
    }

    let (v, _u) = parents.iter().rev().fold((v, u), |(v, u), ps| {
        if ps[v] == ps[u] {
            (v, u)
        } else {
            (ps[v], ps[u])
...
fn main() {
    ...
    dfs(&g, 0, n, &mut idxs, &mut depth, &mut parents[0]);
    ...
    for _ in 0..q {
        ...
        let t = kv
            ...
            .skip(1)
            .map(|v| v - 1)
            .sorted_by_key(|&v| idxs[v])
            .collect::<Vec<_>>();

        let ans = once(t[k - 1])
            .chain(t.citer())
            .tuple_windows()
            .map(|(v, u)| lca(v, u, &depth, &parents))
            .zip(t.citer())
            .map(|(p, v)| depth[v] - depth[p])
            .sum::<usize>();
