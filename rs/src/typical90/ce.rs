...
fn main() {
    ...
    let has_many = (0..n)
        .map(|i| g.out_edges[i].len() >= L)
        .collect::<Vec<_>>();

    let g1 = Graph::from_edges_directed(
        n,
        (0..n).flat_map(|i| {
            if has_many[i] {
                vec![]
    ...
    let g2 = Graph::from_edges_directed(
        n,
        (0..n).flat_map(|i| {
            if has_many[i] {
                g.out_edges[i].clone()
    ...
    let q = read::<usize>();
    let mut colors0 = vec![(0, 1); n];
    let mut colors1 = vec![(0, 1); n];
    for i in 1..=q {
        let (x, y) = read_tuple!(usize, usize);
        let x = x - 1;

        let (_, c) = g2.in_edges[x]
            .citer()
            .map(|e| colors0[e.from])
            .chain(once(colors1[x]))
            .max()
            .unwrap();
        println!("{}", c);
        ...
        colors1[x] = (i, y);
        if !has_many[x] {
            g1.out_edges[x].citer().map(|e| e.to).for_each(|u| {
                colors1[u] = (i, y);
            });
        }
    }
