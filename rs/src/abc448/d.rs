fn main() {
    ...
    dfs(&g, 0, n, &a, &mut FxHashMap::default(), &mut ans);
...
fn dfs(
    ...
    *counts.entry(a[v]).or_insert(0) += 1;
    ...
    ans[v] = (p < ans.len() && ans[p]) || counts[&a[v]] >= 2;

    g.children(v, p).for_each(|u| dfs(g, u, v, a, counts, ans));

    *counts.get_mut(&a[v]).unwrap() -= 1;
}
