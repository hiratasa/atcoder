...
fn main() {
    ...
    let xs = x.citer().map(|x| x - 1).collect::<FxHashSet<_>>();

    let mut uf = (0..m)
        .filter(|&i| !xs.contains(&i))
        .map(|i| (uv[i].0 - 1, uv[i].1 - 1))
    ...
    let counts = x
        .citer()
        .map(|xx| (uv[xx - 1].0 - 1, uv[xx - 1].1 - 1))
        .fold(vec![0; n], |mut counts, (u, v)| {
            counts[uf.root(u)] += 1;
            counts[uf.root(v)] += 1;
            counts
        });

    let l = counts.citer().filter(|&c| c % 2 > 0).count();
