...
fn main() {
    ...
    let mut st = ST::new(n + 1);
    for i in 0..=n {
        if i == 0 {
            st.set(i, (0, i));
        } else {
            st.set(i, (usize::MAX, i));
        }
    }
    ...
    let p = s
        ...
        .scan(st, |st, (i, c)| {
            if c == '1' {
                ...
                let (x, y) = st.query(i.saturating_sub(m), i);
                st.set(i, (x.saturating_add(1), i));
    ...
    let t = successors(Some(n), |&q| q.checked_sub(1).and_then(|qq| p[qq])).collect::<Vec<_>>();
