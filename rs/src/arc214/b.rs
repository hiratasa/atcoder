fn main() {
    input! {
        cases: [(usize, [(Usize1, Usize1, usize)])]
    };
    ...
    cases
        ...
        .map(|(n, abx)| {
            ...
            let g = Graph::from_edges_undirected(n, abx);

            let mut labels = vec![usize::MAX; n];
            dfs(&g, 0, 0, &mut labels);

            let x = labels.iter().copied().fold(0, |x, y| x ^ y);
            let y = (1..=n).fold(0, |x, y| x ^ y);
...
fn dfs(g: &Graph<usize>, v: usize, c: usize, labels: &mut [usize]) {
    if labels[v] != usize::MAX {
        return;
    }
    labels[v] = c;

    g.out_edges[v].iter().copied().for_each(|e| {
        dfs(g, e.to, c ^ e.label, labels);
    });
}
