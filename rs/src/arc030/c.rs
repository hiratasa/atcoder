...
fn main() {
    ...
    let (components, _, edges) = scc(&g);

    let freqs = components
        .iter()
        .map(|idxs| {
            idxs.citer().fold(vec![0; 26], |mut freqs, i| {
                freqs[c[i] as usize - 'a' as usize] += 1;
                freqs
            })
        })
        .collect::<Vec<_>>();
    ...
    let dp = (0..l).rev().fold(
        vec![vec![None; k + 1]; l],
        |mut dp: Vec<Vec<Option<Vec<usize>>>>, v| {
            let empty = vec![];
            for i in 1..=k {
                if let Some(s) = freqs[v]
                    .citer()
                    .enumerate()
                    .flat_map(|(c, t)| itertools::repeat_n(c, t))
                    .chain(once(26))
                    .take(i + 1)
                    .enumerate()
                    .find_map(|(j, c)| {
                        edges[v]
                            .citer()
                            .filter_map(|u| dp[u][i - j].as_ref())
                            .filter(|&s| s[0] < c)
                            .chain(Some(&empty).filter(|_| i == j))
                            .min()
                    })
                {
                    dp[v][i] = Some(
    ...
    println_opt(
        dp.iter()
            .filter_map(|t| t[k].as_ref())
            .min()
            .map(|s| s.citer().map(|c| ('a' as usize + c) as u8 as char).join("")),
    );
}
