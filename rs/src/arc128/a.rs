...
define_monoid!(Maximum, (usize, usize), (0, 0), std::cmp::max);

type ST = SegmentTree<Maximum>;

fn solve(st: &ST, l: usize, r: usize, first_idx: usize, last_idx: usize, ans: &mut [usize]) {
    // eprintln!("{} {}", l, r);
    ...
    if i == l {
        ...
        solve(st, l, r - 1, first_idx, 0, ans);
    } else {
        ans[i] = 1;
        solve(st, l, i, first_idx, 0, ans);
        solve(st, i + 1, r, 1, last_idx, ans);
    }
}
...
fn main() {
    ...
    let st = ST::with(&a.citer().enumerate().map(|(i, aa)| (aa, i)).collect());
    let mut ans = vec![0; n];
    solve(&st, 0, n, 0, 0, &mut ans);
