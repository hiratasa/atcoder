...
fn dfs(g: &Graph, v: usize, p: usize, visited: &mut [bool]) -> Option<(usize, usize)> {
    if visited[v] {
        return Some((v, p));
    }
    ...
    g.children(v, p).find_map(|u| dfs(g, u, v, visited))
}

fn mark_path(g: &Graph, v: usize, p: usize, a: usize, b: usize, marked: &mut [bool]) -> bool {
    if v == b {
        marked[v] = true;
        return true;
    }
    ...
    marked[v] = g
        .children(v, p)
        .filter(|&u| (u, v) != (a, b) && (u, v) != (b, a))
        .any(|u| mark_path(g, u, v, a, b, marked));
...
fn calc_path_parent(
    ...
    marked: &[bool],
    pp: Option<usize>,
    path_parent: &mut [usize],
) {
    let pp = if marked[v] { Some(v) } else { pp };
    ...
    g.children(v, p)
        .filter(|&u| (u, v) != (a, b) && (u, v) != (b, a))
        .for_each(|u| calc_path_parent(g, u, v, a, b, marked, pp, path_parent));
}

fn main() {
    let n = read::<usize>();
    let uv = read_vec(n, || read_tuple!(usize, usize));

    let g = Graph::from_edges1_undirected(n, uv);

    let (a, b) = dfs(&g, 0, n, &mut vec![false; n]).unwrap();

    let mut on_path = vec![false; n];
    mark_path(&g, 0, n, a, b, &mut on_path);
    let mut path_parent = vec![a; n];
    calc_path_parent(&g, 0, n, a, b, &on_path, None, &mut path_parent);
