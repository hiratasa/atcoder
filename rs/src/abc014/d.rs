...
fn dfs(g: &Graph, v: usize, depth: &mut [usize], parents: &mut [usize]) {
    g.out_edges[v].iter().map(|e| e.to).for_each(|u| {
        if u != parents[v] {
            depth[u] = depth[v] + 1;
            parents[u] = v;
            dfs(g, u, depth, parents);
        }
    })
}
...
fn main() {
    ...
    let g = Graph::from_edges1_undirected(n, xy.citer());
    ...
    dfs(&g, 0, &mut depth, &mut parents[0]);
    ...
    for (a, b) in ab {
        ...
        let p = lca(&depth, &parents, a, b);

        let ans = depth[a] + depth[b] - 2 * depth[p] + 1;
        println!("{}", ans);
    }
}
