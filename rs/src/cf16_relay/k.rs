...
fn dfs(g: &Graph, v: usize, p: usize) -> (usize, usize) {
    ...
    if t.len() == 0 {
        ...
        let x = t[0].0 + t[1].0;
        (t[0].0, max(x, t.citer().map(|(_, y)| y).max().unwrap()))
    }
}
...
fn main() {
    ...
    let g = Graph::from_edges1_undirected(n, pq);

    let c = (0..n).find(|&i| g.out_edges[i].len() == 1).unwrap();
    let (x, y) = dfs(&g, c, n);

    let y = max(x - 1, y);

    let k = (0..n).filter(|&i| g.out_edges[i].len() == 1).count();
