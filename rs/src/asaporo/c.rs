...
fn dfs(
    ...
    g.children_edge(v, p).for_each(|e| {
        ...
        dfs(g, e.to, v, parents, maxs, depth)
    })
}
...
fn main() {
    ...
    let sum = tree_edges.citer().map(|(_, _, c)| c).sum::<usize>();

    let g = Graph::from_edges_undirected(n, tree_edges);

    let mut depth = vec![0; n];
    let mut parents = vec![vec![0; n]; 20];
    let mut maxs = vec![vec![0; n]; 20];
    dfs(&g, 0, n, &mut parents[0], &mut maxs[0], &mut depth);

    for i in 1..20 {
        for v in 0..n {
            parents[i][v] = parents[i - 1][parents[i - 1][v]];
            maxs[i][v] = max(maxs[i - 1][parents[i - 1][v]], maxs[i - 1][v]);
        }
    }
    ...
    st.citer()
        .map(|(s, t)| {
            ...
            let (mut s, mut t) = if depth[s] < depth[t] { (s, t) } else { (t, s) };

            let mut k = 0;

            for i in (0..20).rev() {
                if depth[parents[i][t]] >= depth[s] {
                    k = max(k, maxs[i][t]);
                    t = parents[i][t];
                }
            }
            ...
            if s != t {
                ...
                assert!(parents[0][s] == parents[0][t]);
