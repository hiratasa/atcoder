...
fn main() {
    ...
    let abcd = read_vec(n, || read_tuple!(usize, usize, usize, usize));

    let mut g = MaxFlowGraph::new(h + w + 2 * n + 2);
    let src = h + w + 2 * n;
    let dst = h + w + 2 * n + 1;
    ...
    for i in 0..w {
        g.add_edge(h + i, dst, 1);
    }

    for i in 0..n {
        for hh in abcd[i].0 - 1..abcd[i].2 {
            g.add_edge(hh, h + w + i, 1);
        }

        g.add_edge(h + w + i, h + w + n + i, 1);

        for ww in abcd[i].1 - 1..abcd[i].3 {
            g.add_edge(h + w + n + i, h + ww, 1);
        }
    }
