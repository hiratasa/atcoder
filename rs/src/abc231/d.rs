...
fn has_cycle(g: &Graph, v: usize, p: usize, visited: &mut [bool], visiting: &mut [bool]) -> bool {
    if visiting[v] {
        return true;
    }
    ...
    if g.out_edges[v]
        ...
        .map(|e| e.to)
        .filter(|&u| u != p)
        .any(|u| has_cycle(g, u, v, visited, visiting))
    {
        return true;
    }
...
fn main() {
    ...
    if (0..n).any(|i| g.out_edges[i].len() > 2) {
        println!("No");
        return;
    }
    ...
    for i in 0..n {
        if !visited[i] && has_cycle(&g, i, n, &mut visited, &mut visiting) {
            println!("No");
            return;
        }
    }
