...
define_group!(Sum, usize, 0, usize::wrapping_add, usize::wrapping_neg);
...
fn main() {
    ...
    let a_values = a.citer().sorted().dedup().collect::<Vec<_>>();
    let a_idxs = a_values
        .citer()
        .enumerate()
        .map(|(i, aa)| (aa, i))
        .collect::<FxHashMap<_, _>>();
    let c = a.citer().map(|aa| a_idxs[&aa]).collect::<Vec<_>>();

    let b_values = b.citer().sorted().dedup().collect::<Vec<_>>();
    let b_idxs = b_values
        .citer()
        .enumerate()
        .map(|(i, aa)| (aa, i))
        .collect::<FxHashMap<_, _>>();
    let d = b.citer().map(|aa| b_idxs[&aa]).collect::<Vec<_>>();

    let cd = izip!(c, d)
        .sorted_by_key(|&(c, d)| (c, Reverse(d)))
        .collect::<Vec<_>>();
    ...
    let ans = cd
        ...
        .into_iter()
        .map(|(x, it)| (x, it.count()))
        .scan(BIT::<Sum>::new(n), |bit, ((c, d), k)| {
            let m = /* self */ k * k + k * bit.sum_between(d, n);
