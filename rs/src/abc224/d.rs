...
fn main() {
    ...
    let adjs = uv.citer().fold(vec![vec![]; 9], |mut adjs, (u, v)| {
        adjs[u - 1].push(v - 1);
        adjs[v - 1].push(u - 1);
        adjs
    });

    let perms = (0..9).permutations(9).collect::<Vec<_>>();
    let idxs = perms
        .iter()
        .enumerate()
        .map(|(i, v)| (v, i))
    ...
    let g = Graph::from_edges_directed(
        n,
        perms.iter().enumerate().flat_map(|(idx, perm)| {
            let x = perm[8];

            let idxs = &idxs;
            adjs[x].citer().map(move |y| {
                let i = perm.citer().position(|z| z == y).unwrap();
    ...
    let v0 = (1..=9).find(|&x| !p.contains(&x)).unwrap() - 1;

    let src = idxs[&p.citer().map(|x| x - 1).chain(once(v0)).collect::<Vec<_>>()];
    let dst = idxs[&(0..9).collect::<Vec<_>>()];
    ...
    fn chmin(x: &mut usize, y: usize) -> bool {
        if y < *x {
            *x = y;
            true
        } else {
