fn main() {
    ...
    let mut st = SegmentTree::<Max>::new(w);
    st.set(0, (0, usize::MAX));

    let (prevs, st) =
        (0..n)
            .sorted_by_key(|&i| rc[i])
            .fold((vec![0; n], st), |(mut prevs, mut st), i| {
                let (_r, c) = rc[i];
                let (d, p) = st.query(0..=c).0;
                prevs[i] = p;
                st.set(c, (d + 1, i));
    ...
    let (ans, p0) = st.query(0..w).0;

    let path = once((h - 1, w - 1))
        .chain(
            successors(Some(p0), |&p| Some(prevs[p]).filter(|&pp| pp != usize::MAX)).map(|i| rc[i]),
        )
        .chain(once((0, 0)))
        .collect::<Vec<_>>()
        .into_iter()
        .rev()
        .tuple_windows()
        .flat_map(|((i0, j0), (i1, j1))| repeat_n('D', i1 - i0).chain(repeat_n('R', j1 - j0)))
        .collect::<Vec<_>>();
...
define_monoid!(Max, (i64, usize), (i64::MIN, usize::MAX), std::cmp::max);
