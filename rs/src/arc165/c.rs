...
fn dfs(g: &Graph, v: usize, visited: &mut [bool], color: &mut [bool], c: bool, x: usize) -> bool {
    if visited[v] {
        return color[v] == c;
    }
    color[v] = c;
    ...
    g.out_edges[v]
        .citer()
        .take_while(|e| e.label < x)
        .all(|e| dfs(g, e.to, visited, color, !c, x))
}

fn main() {
    input! {
        n: usize, m: usize, abw: [(Usize1, Usize1, usize); m]
    }

    let g = Graph::from_edges_undirected(n, abw.citer().sorted_by_key(|&(_, _, w)| w));

    let ans = lower_bound_int(1, 1 << 40, |x| {
        let mut visited = vec![false; n];
        let mut color = vec![false; n];
        if (0..n).any(|i| !visited[i] && !dfs(&g, i, &mut visited, &mut color, false, x)) {
            Ordering::Greater
        } else {
            if (0..n)
                .filter_map(|i| {
                    let (w0, w1) = g.out_edges[i]
                        .citer()
                        .map(|e| e.label)
                        .take_while(|&w| w < x)
                        .next_tuple()?;
