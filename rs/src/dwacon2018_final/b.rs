...
fn main() {
    ...
    let sts = v.citer().fold(
        vec![SegmentTree::<Min>::with(&vvec![-1; i64::MAX; n + 1]); k + 1],
        |mut sts, x| {
            for i in (0..=k).rev() {
                if i < k {
                    let idx = sts[i].left_partition_point(n + 1, |y| y.0 == i64::MAX);
                    if idx <= n {
                        let z = min(sts[i + 1].get(idx).0, x);
                        sts[i + 1].set(idx, z);
                    }
                }
                let idx = sts[i].left_partition_point(n + 1, |y| y.0 >= x);
                if idx <= n {
                    sts[i].set(idx, x);
                }
            }
    ...
    let ans = sts
        .iter()
        .map(|st| st.left_partition_point(n + 1, |y| y.0 == i64::MAX) - 1)
        .max()
        .unwrap();
