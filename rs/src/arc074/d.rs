...
fn main() {
    ...
    let (ti, tj) = a
        .iter()
        .enumerate()
        .find_map(|(i, row)| row.citer().position(|c| c == 'T').map(|j| (i, j)))
        .unwrap();
    ...
    g.add_edge(src, si, usize::MAX);
    g.add_edge(src, h + sj, usize::MAX);
    g.add_edge(ti, dst, usize::MAX);
    g.add_edge(h + tj, dst, usize::MAX);
