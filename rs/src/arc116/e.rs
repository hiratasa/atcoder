...
fn dfs(g: &Graph, v: usize, p: usize, t: usize) -> (Result<usize, usize>, usize) {
    let (a, b, s) = g.out_edges[v]
        .iter()
        .map(|e| e.to)
        .filter(|&u| u != p)
        .map(|u| dfs(g, u, v, t))
        .fold((None, 0usize, 0usize), |(a, b, s), (res, ss)| match res {
            Ok(c) => {
                if c >= 1 {
                    (max(Some(c - 1), a), b, s + ss)
                } else {
                    (a, b, s + ss)
                }
            }
            Err(d) => (a, max(b, d + 1), s + ss),
        });
...
fn main() {
    ...
    let ans = lower_bound_int(0, n, |t| {
        let s = match dfs(&g, 0, n, t) {
            (Ok(_), s) => s,
            (Err(_), s) => s + 1,
        };
