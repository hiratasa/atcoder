...
fn main() {
    ...
    let abc = read_vec(m, || read_tuple!(usize, usize, char));
    let s = read_str();

    let g =
        Graph::from_edges1_undirected(n, abc.citer().enumerate().map(|(i, (a, b, _))| (a, b, i)));
    let colors = abc.citer().map(|(_, _, c)| c).collect::<Vec<_>>();
    ...
    for i in 0..m {
        ...
        if !uf.same(a, b) && colors[i] == s[a] && colors[i] == s[b] {
            selected[i] = true;
            uf.unite(a, b);
            ok[a] = true;
            ok[b] = true;
    ...
    for i in 0..n {
        ...
        edges.extend(g.out_edges[i].citer().map(|e| e.label));
    }

    while let Some(i_edge) = edges.pop() {
        let (a, b, c) = abc[i_edge];
        let a = a - 1;
        let b = b - 1;
        ...
        assert!(!selected[i_edge]);

        if uf.size(a) > 1 && uf.size(b) > 1 {
            continue;
        }

        let (v, _) = if uf.size(a) == 1 { (a, b) } else { (b, a) };

        if s[v] == colors[i_edge] {
            selected[i_edge] = true;
            uf.unite(a, b);
            edges.extend(g.out_edges[v].citer().map(|e| e.label));
            ok[v] = true;
        }
    }
    ...
    println!(
        "{}",
        selected.citer().positions(|o| o).map(|i| i + 1).join(" ")
    );
}
