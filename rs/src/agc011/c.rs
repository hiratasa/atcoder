...
fn dfs(g: &Graph, v: usize, visited: &mut [bool], colors: &mut [bool]) -> (bool, usize) {
    visited[v] = true;
    ...
    for u in g.adjs(v) {
        ...
        colors[u] = !colors[v];
        let (is_bipartite2, size2) = dfs(&g, u, visited, colors);

        is_bipartite &= is_bipartite2;
        size += size2;
    }
...
fn main() {
    ...
    let t = (0..n)
        .scan((vec![false; n], vec![false; n]), |(visited, colors), i| {
            if visited[i] {
                Some(None)
            } else {
                match dfs(&g, i, visited, colors) {
                    (_, 1) => Some(Some(0)),
                    (true, _) => Some(Some(1)),
                    (false, _) => Some(Some(2)),
                }
        ...
        .flatten()
        .fold(vec![0; 3], |mut t, x| {
            t[x] += 1;
            t
        });

    let ans = (2 * t[0] * n - t[0] * t[0]) + 2 * t[1] * t[1] + 2 * t[1] * t[2] + t[2] * t[2];
    println!("{}", ans);
}
