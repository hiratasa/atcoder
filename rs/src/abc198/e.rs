...
fn dfs(
    ...
    p: usize,
    c: &[usize],
    m: &mut FxHashMap<usize, usize>,
    a: &mut Vec<usize>,
) {
    if m.get(&c[v]).copied().unwrap_or(0) == 0 {
        a.push(v);
    }

    *m.entry(c[v]).or_insert(0) += 1;

    g.out_edges[v]
        .iter()
        .map(|e| e.to)
        .filter(|&u| u != p)
        .for_each(|u| dfs(g, u, v, c, m, a));
...
fn main() {
    ...
    let mut ans = vec![];
    dfs(&g, 0, n, &c, &mut FxHashMap::default(), &mut ans);

    for a in ans.citer().sorted() {
        println!("{}", a + 1);
    }
}
