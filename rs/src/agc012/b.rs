...
fn main() {
    ...
    let q = read::<usize>();
    let vdc = read_vec(q, || read_tuple!(usize, usize, usize));
    ...
    let (t, _) = vdc
        .citer()
        .enumerate()
        .map(|(i_query, (v, d, c))| (d, v - 1, i_query + 1, c))
        .sorted_by_key(|&(d, _, _, _)| Reverse(d))
        // dummy to ensure existence of d==0
        .chain(once((0, 0, 0, 0)))
        .group_by(|&(d, _, _, _)| d)
        .into_iter()
        .fold((vec![(0, 0); n], 10), |(prev, e), (d, it)| {
            let mut next = prev;
            for _ in d..e {
                let prev = next.clone();
                for v in 0..n {
                    for u in g.adjs(v) {
                        next[u] = max(next[u], prev[v]);
                    }
                }
            }

            for (_, v, i_query, c) in it {
                next[v] = max(next[v], (i_query, c));
            }
