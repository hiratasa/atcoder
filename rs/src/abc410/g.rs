fn main() {
    ...
    let t = ab
        ...
        .sorted_by_key(|&(a, _)| a)
        .fold(vec![vec![]; 2 * n + 1], |mut t, (a, b)| {
            t[b].push(a);
            t
        });

    let mut st = SegmentTree::<Max>::new(2 * n + 1);
    let mut t0 = vec![0; 2 * n + 1];
    for i in 0..=2 * n {
        for &j in &t[i] {
            let x = st.query(j + 1..=2 * n).0;
            st.set(j, x + 1);
        }
        t0[i] = st.query(0..=2 * n).0;
    }

    let ans = (0..=2 * n)
        .map(|i| t0[i] + st.query(i + 1..=2 * n).0)
        .chain(once(t0[2 * n]))
        .max()
        .unwrap();
