...
fn main() {
    ...
    let wl = read_vec(m, || read_tuple!(usize, usize));

    let mat =
        wl.citer()
            .map(|(w, l)| (w - 1, l - 1))
            .fold(vec![vec![None; n]; n], |mut mat, (w, l)| {
                mat[w][l] = Some(true);
                mat[l][w] = Some(false);
    ...
    let ans = (0..n)
        .filter(|&i| {
            let mut g = MaxFlowGraph::new(n + n * (n - 1) / 2 + 2);
            let src = n + n * (n - 1) / 2;
            let dst = n + n * (n - 1) / 2 + 1;

            let k = mat[i].citer().filter(|&x| x.unwrap_or(true)).count() - /* self */ 1;
            ...
            for j in 0..n {
                if j == i {
                    g.add_edge(src, j, usize::MAX);
                } else {
                    g.add_edge(src, j, k - 1);
                }
            }
            ...
            for j in 0..n {
                for l in 0..j {
                    ...
                    if mat[j][l].unwrap_or(l != i) {
                        g.add_edge(j, idx, 1);
                    }
                    if mat[l][j].unwrap_or(j != i) {
                        g.add_edge(l, idx, 1);
                    }
                }
            }
