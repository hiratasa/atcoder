...
fn main() {
    ...
    let sum_row = a
        ...
        .collect::<Vec<_>>();
    let sum_col = (0..w)
        .map(|i| a.iter().map(|row| row[i]).sum::<i64>())
        .collect::<Vec<_>>();

    let mut g = MaxFlowGraph::new(h + w + 2);
    let src = h + w;
    let dst = h + w + 1;

    for i in 0..h {
        if sum_row[i] > 0 {
            g.add_edge(src, i, sum_row[i].abs() as usize);
        } else {
            g.add_edge(i, dst, sum_row[i].abs() as usize);
        }
    }
    for i in 0..w {
        if sum_col[i] > 0 {
            g.add_edge(h + i, dst, sum_col[i].abs() as usize);
        } else {
            g.add_edge(src, h + i, sum_col[i].abs() as usize);
        }
    }
    ...
    for i in 0..h {
        for j in 0..w {
            if a[i][j] >= 0 {
                g.add_edge(i, h + j, a[i][j] as usize);
            } else {
                g.add_edge(i, h + j, 1usize << 60);
            }
        }
    }
    ...
    let ans = sum_row.citer().map(|x| max(x, 0)).sum::<i64>()
        + sum_col.citer().map(|x| max(x, 0)).sum::<i64>()
        - f as i64;
