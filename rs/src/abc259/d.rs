...
fn main() {
    ...
    let xyr = read_vec(n, || read_tuple!(i64, i64, i64));

    let s = xyr
        .citer()
        .position(|(x, y, r)| (x - sx).pow(2) + (y - sy).pow(2) == r * r)
        .unwrap();
    let t = xyr
        .citer()
        .position(|(x, y, r)| (x - tx).pow(2) + (y - ty).pow(2) == r * r)
        .unwrap();

    let mut uf = xyr.citer().enumerate().tuple_combinations().fold(
        UnionFind::new(n),
        |mut uf, ((i0, (x0, y0, r0)), (i1, (x1, y1, r1)))| {
            let d2 = (x0 - x1).pow(2) + (y0 - y1).pow(2);
            let r = max(r0, r1) - min(r0, r1);
            let rr = r0 + r1;
            if r * r <= d2 && d2 <= rr * rr {
                uf.unite(i0, i1);
            }
