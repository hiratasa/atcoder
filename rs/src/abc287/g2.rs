    ...
    Op: Monoid + Operator<M::Item>,
{
    ...
    // 全てのlowerに対してtrueとなる場合は (None, 最小のkey, 全区間の値) を返す
    // 全てのlowerに対してfalseとなる場合はエラー(f(&M::id())がtrueである必要がある)
    ...
    // 全てのupperに対してtrueとなる場合は (Some(最大のkey), None, 全区間の値) を返す
    // 全てのupperに対してfalseとなる場合は (None, Some(lower), M::id()) を返す
    fn right_partition_point<F>(&mut self, lower: &K, mut f: F) -> (Option<K>, Option<K>, M::Item)
    where
        F: FnMut(&M::Item) -> bool,
    {
        if !f(&M::id()) {
            return (None, Some(lower.clone()), M::id());
        }
...
type T = Treap<(usize, usize), M, NullMonoid>;
    ...
    query
        ...
        .scan((t, s), |(t, s), qq| match qq[0] {
            1 => {
                ...
                t.remove(&(ab[i].0, i));
                ab[i].0 = a;
                t.insert_with_value((ab[i].0, i), (ab[i].1, ab[i].0 * ab[i].1));
            ...
            3 => {
                ...
                if x > *s {
                    ...
                    let (tmp, _, (m, z)) =
                        t.left_partition_point(&(usize::MAX, 0), |&(m, _)| m < x);
                    let (k, _) = tmp.unwrap();
