...
fn main() {
    ...
    let g = Graph::from_edges_directed(
        n,
        edges.citer().enumerate().map(|(i, (u, v, _, _))| (u, v, i)),
    );

    let ans = lower_bound(0.0, 2e4, 1e-10, |w: f64| {
        let mut table = vec![f64::MIN; n];
        ...
        for i in 0..n {
            g.out_edges[i]
                .citer()
                .map(|e| {
                    let (_, _, b, c) = edges[e.label];

                    (e.to, b - c * w)
                })
                .for_each(|(to, val)| {
                    table[to] = table[to].max(table[i] + val);
                });
        }
