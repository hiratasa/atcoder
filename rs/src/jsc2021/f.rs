...
type ST = SegmentTree<Value>;

fn main() {
    let (n, m, q) = read_tuple!(usize, usize, usize);
    let nm = vec![n, m];

    let queries = read_vec(q, || read_tuple!(usize, usize, usize));

    let mut idxs = once((0, 0))
        .chain(queries.citer().map(|a| (a.2, 0)))
        .collect::<BTreeMap<_, _>>();
    idxs.iter_mut().enumerate().for_each(|(i, a)| {
        *a.1 = i;
    });
    ...
    queries
        ...
        .scan(
            (
                ...
                vec![vec![0; n], vec![0; m]],
                vec![
                    ST::with(&vvec![(n, 0); (0, 0); k]),
                    ST::with(&vvec![(m, 0); (0, 0); k]),
                ],
            ),
            |(z, ab, st), (t, x, y)| {
                let t = t - 1;
                ...
                let old = ab[t][x];
                let old_idx = idxs[&old];

                ab[t][x] = y;
                let new_idx = idxs[&y];

                let tmp = st[t].get(old_idx);
                st[t].set(old_idx, (tmp.0 - 1, tmp.1 - old));
                let tmp = st[t].get(new_idx);
                st[t].set(new_idx, (tmp.0 + 1, tmp.1 + y));

                let (num0, sum0) = st[u].query(old_idx, k);
                let (num1, sum1) = st[u].query(new_idx, k);

                *z = *z - (sum0 + (nm[u] - num0) * old) + (sum1 + (nm[u] - num1) * y);
