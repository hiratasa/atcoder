...
fn is_prime(x: usize) -> bool {
    assert!(x > 1);
    (2..).take_while(|&y| y * y <= x).all(|y| x % y > 0)
}
...
fn main() {
    ...
    ab.citer()
        .enumerate()
        .tuple_combinations()
        .filter(|&((_, (a0, _)), (_, (a1, _)))| is_prime(a0 + a1))
        .for_each(|((i0, (a0, _)), (i1, (a1, _)))| {
            if a0 % 2 == 0 {
                assert!(a1 % 2 > 0);
                g.add_edge(i0, i1, usize::MAX);
            } else {
                assert!(a1 % 2 == 0);
                g.add_edge(i1, i0, usize::MAX);
            }
        });
    ...
    let f1 = if let Some(idx1) = ab.citer().position(|(a, _)| a == 1) {
        let b1 = ab[idx1].1;

        let b = lower_bound_int(0, b1 + 1, |b| {
            let mut g = g0.clone();

            g.caps[edge_idxs[idx1]] = b;

            if g.max_flow(src, dst) == f {
                Ordering::Greater
            } else {
                Ordering::Less
            }
