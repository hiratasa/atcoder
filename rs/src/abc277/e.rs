...
fn main() {
    ...
    let uva = read_vec(m, || read_tuple!(usize, usize, usize));
    let s = read_row::<usize>();

    let g = Graph::from_edges_undirected(
        2 * n,
        chain(
            uva.citer().map(|(u, v, a)| (u - 1 + a * n, v - 1 + a * n)),
            s.citer().map(|x| (x - 1, x - 1 + n)),
        ),
    );

    let mut q = VecDeque::new();
    let mut costs = vec![usize::MAX; 2 * n];
    q.push_back((0, n));
    costs[n] = 0;

    while let Some((cost, v)) = q.pop_front() {
        if cost != costs[v] {
            continue;
        }
        ...
        g.adjs(v)
            ...
            .filter(|&(u, next_cost)| chmin(&mut costs[u], next_cost))
            .for_each(|(u, next_cost)| {
                if next_cost == cost {
                    q.push_front((next_cost, u));
                } else {
                    q.push_back((next_cost, u))
                }
            });
    }
