fn main() {
    ...
    let ans = p
        ...
        .scan(
            (
                vec![(usize::MAX, 0), (p[0], 0)],
                SegmentTree::<Min>::with(&vec![(a[p[0]], 0); n]),
                vec![0; n],
            ),
            |(maxs, st, nums), (i, idx)| {
                while let Some(&(x, _j)) = maxs.last()
                    && x < idx
                {
                    maxs.pop();
                }

                let (cost, pi) = st.query(maxs[maxs.len() - 1].1..i).0;
                let parent = p[pi];
                ...
                nums[parent] += 1;
                st.set(pi, (a[parent] * (2 * nums[parent] + 1), pi));
...
define_monoid!(Min, (usize, usize), (usize::MAX, usize::MAX), std::cmp::min);
