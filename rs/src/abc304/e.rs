...
fn main() {
    ...
    let k = read::<usize>();
    let xy = read_vec(k, || read_tuple!(usize, usize));
    let q = read::<usize>();
    let pq = read_vec(q, || read_tuple!(usize, usize));

    let mut uf = uv.citer().fold(UnionFind::new(n), |mut uf, (u, v)| {
        uf.unite(u - 1, v - 1);
        uf
    });
    ...
    let set = xy
        .citer()
        .map(|(x, y)| (uf.root(x - 1), uf.root(y - 1)))
        .map(|(x, y)| (min(x, y), max(x, y)))
        .collect::<FxHashSet<_>>();

    pq.citer()
        .map(|(p, q)| (uf.root(p - 1), uf.root(q - 1)))
        .map(|(p, q)| (min(p, q), max(p, q)))
        .map(|(p, q)| !set.contains(&(p, q)))
        .for_each(|ans| {
            if ans {
                println!("Yes");
            } else {
