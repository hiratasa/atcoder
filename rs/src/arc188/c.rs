fn main() {
    ...
    let g =
        Graph::from_edges_undirected(2 * n, abc.into_iter().map(|(a, b, c)| (n + a, b, c == 1)));

    let mut colors = vec![None; 2 * n];
    for i in 0..2 * n {
        if colors[i].is_none() {
            if !dfs(&g, i, false, &mut colors) {
                println!("-1");
                return;
            }
        }
    ...
    println!(
        "{}",
        (0..n)
            .map(|i| (colors[i].unwrap() ^ colors[n + i].unwrap()) as usize)
            .join("")
    );
}

fn dfs(g: &Graph<bool>, v: usize, color: bool, colors: &mut [Option<bool>]) -> bool {
    if colors[v] == Some(!color) {
        return false;
    }
    if colors[v].is_some() {
        return true;
    ...
    for e in &g.out_edges[v] {
        if !dfs(g, e.to, color ^ e.label, colors) {
            return false;
        }
    }
