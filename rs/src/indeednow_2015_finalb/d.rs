...
fn main() {
    ...
    let (sx, sy) = read_tuple!(usize, usize);
    let (gx, gy) = read_tuple!(usize, usize);
    let p = read_mat::<usize>(h);
    ...
    let ans1 = iproduct!(0..h, 0..w)
        .flat_map(|(i, j)| {
            it![(usize::MAX, 0), (1, 0), (0, usize::MAX), (0, 1)]
                ...
                .map(move |(ni, nj)| (p[i][j] * p[ni][nj], i * w + j, ni * w + nj))
        })
        .sorted_by_key(|&(c, _, _)| Reverse(c))
        .scan(UnionFind::new(h * w), |uf, (c, id, id2)| {
            if !uf.same(id, id2) {
                uf.unite(id, id2);
                Some(c)
            } else {
