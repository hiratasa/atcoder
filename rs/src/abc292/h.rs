...
define_monoid!(PrefixSumMax, (i64, i64), (0, i64::MIN), |lhs: (
    i64,
    i64
),
                                                         rhs: (
    ...
    (lhs.0 + rhs.0, max(lhs.1, lhs.0.saturating_add(rhs.1)))
});

fn main() {
    let (n, b, q) = read_tuple!(usize, i64, usize);
    let a = read_row::<i64>();
    let cx = read_vec(q, || read_tuple!(usize, i64));

    cx.citer()
        .scan(
            SegmentTree::<PrefixSumMax>::with(
                &a.citer().map(|x| (x - b, x - b)).collect::<Vec<_>>(),
            ),
            |st, (c, x)| {
                st.set(c - 1, (x - b, x - b));

                let r = st
                    .right_partition_point(0, |&PrefixSumMax((_, ma))| ma < 0)
                    .unwrap_or(n);

                let t = st.query(0..r).0.0;

                Some((t + r as i64 * b) as f64 / r as f64)
            },
        )
        .for_each(|ans| {
            println!("{}", ans);
