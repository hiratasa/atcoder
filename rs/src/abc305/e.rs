...
fn main() {
    ...
    let ph = read_vec(k, || read_tuple!(usize, usize));

    let g = Graph::from_edges1_undirected(n, ab);

    let mut rem: Vec<Option<usize>> = vec![None; n];
    let mut q = VecDeque::new();

    let mut ph = ph;
    ph.sort_by_key(|&(_p, h)| h);

    while let Some((p0, h0)) = ph.pop() {
        let p0 = p0 - 1;
        if matches!(rem[p0], Some(h) if h >= h0) {
            continue;
        }
        assert!(rem[p0].is_none());
        rem[p0] = Some(h0);
        q.push_back((p0, h0));

        while let Some((v, hh)) = q.pop_front() {
            while matches!(ph.last(), Some(&(_, h)) if h == hh) {
                let (p, h) = ph.pop().unwrap();
                let p = p - 1;
                if matches!(rem[p], Some(hhh) if hhh >= h) {
                    continue;
                }
                assert!(rem[p].is_none());
                rem[p] = Some(h);
                q.push_back((p, h));
            }
            ...
            for u in g.adjs(v) {
                if matches!(rem[u], Some(hhh) if hhh >= hh - 1) {
                    continue;
                }
                assert!(rem[u].is_none());
                rem[u] = Some(hh - 1);
                q.push_back((u, hh - 1));
            }
        }
    ...
    let ans = (0..n).filter(|&i| rem[i].is_some()).collect::<Vec<_>>();
    println!("{}", ans.len());
