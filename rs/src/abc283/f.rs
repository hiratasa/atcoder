...
define_monoid!(Min, i64, 1 << 50, std::cmp::min);
...
fn main() {
    ...
    let ans0 = p
        ...
        .scan(
            (
                SegmentTree::<Min>::new(n + 1),
                SegmentTree::<Min>::new(n + 1),
            ),
            |(st_neg, st_pos), (i, x)| {
                let y_neg = st_neg.query(0, x).0 + x as i64 + i as i64;
                let y_pos = st_pos.query(x, n + 1).0 - x as i64 + i as i64;

                st_neg.set(x, -(i as i64) - x as i64);
                st_pos.set(x, -(i as i64) + x as i64);

                Some(min(y_neg, y_pos))
            },
        )
        .collect::<Vec<_>>();
