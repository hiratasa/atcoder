...
fn dijkstra1(g: &Graph, src: usize, dst: usize) -> Option<usize> {
    ...
    while let Some(std::cmp::Reverse((cost, v))) = q.pop() {
        ...
        for &edge in &g.out_edges[v] {
            let next_cost = (cost + edge.label.1 - 1) / edge.label.1 * edge.label.1 + edge.label.0;
...
fn main() {
    let (n, m, x, y) = read_tuple!(usize, usize, usize, usize);

    let abtk = read_vec(m, || read_tuple!(usize, usize, usize, usize));

    let g = Graph::from_edges1_undirected(n, abtk);

    let ans = dijkstra1(&g, x - 1, y - 1);
    if let Some(ans) = ans {
        println!("{}", ans);
    } else {
        println!("-1");
