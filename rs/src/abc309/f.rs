...
fn main() {
    let n = read::<usize>();
    let hwd = read_vec(n, || read_tuple!(usize, usize, usize));

    let coords = hwd
        .citer()
        .flat_map(|(h, w, d)| it![h, w, d])
        .sorted()
        .dedup()
        .collect::<Vec<_>>();
    let idxs = coords
    ...
    let ans = hwd
        ...
        .sorted_by_key(|&(h, w, _)| (h, Reverse(w)))
        .scan(SegmentTree::<Min>::new(idxs.len()), |st, (_h, w, d)| {
            let idx = idxs[&w];

            let mi = st.query(0..idx).0;
            st.set(idx, min(st.get(idx).0, d));
