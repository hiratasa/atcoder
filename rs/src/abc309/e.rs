...
fn dfs(g: &Graph, v: usize, insurance: &[Option<usize>], c: Option<usize>, marked: &mut [bool]) {
    let c = max(c, insurance[v]);

    marked[v] = c.is_some();

    g.adjs(v).for_each(|u| {
        dfs(g, u, insurance, c.and_then(|c| c.checked_sub(1)), marked);
    });
}
...
fn main() {
    ...
    let g = Graph::from_edges_directed(n, p.citer().enumerate().map(|(i, j)| (j - 1, i + 1)));

    let insurance = xy.citer().fold(vec![None; n], |mut insurance, (x, y)| {
        insurance[x - 1] = max(insurance[x - 1], Some(y));
        insurance
    });

    let mut marked = vec![false; n];
    dfs(&g, 0, &insurance, None, &mut marked);

    let ans = marked.citer().filter(|&x| x).count();
