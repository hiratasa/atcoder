...
fn main() {
    let (n1, n2, m) = read_tuple!(usize, usize, usize);
    let ab = read_vec(m, || read_tuple!(usize, usize));

    let g = Graph::from_edges1_undirected(n1 + n2, ab);

    let bfs = |st: usize| {
        let mut q = VecDeque::new();
        let mut costs = vec![usize::MAX; n1 + n2];
        ...
        costs.citer().filter(|&c| c != usize::MAX).max().unwrap()
    };
