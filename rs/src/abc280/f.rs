...
fn dfs(g: &Graph, v: usize, f: &mut [i64], visited: &mut [usize], vs: &mut Vec<usize>) -> bool {
    vs.push(v);
    ...
    g.out_edges[v]
        ...
        .map(|e| {
            if visited[e.to] > 0 {
                f[e.to] == f[e.from] + e.label
            } else {
                visited[e.to] = visited[e.from];
                f[e.to] = f[e.from] + e.label;
                dfs(g, e.to, f, visited, vs)
            }
        })
        .fold(true, |x, y| x && y)
}
...
fn main() {
    ...
    let g = Graph::from_edges1_directed(
        n,
        abc.citer().flat_map(|(a, b, c)| it![(a, b, c), (b, a, -c)]),
    );

    let (f, idxs, _) = (0..n).fold(
        (vec![0; n], vec![0; n], 1),
        |(mut f, mut visited, idx), v| {
            if visited[v] == 0 {
                visited[v] = idx;

                let mut vs = vec![];
                if !dfs(&g, v, &mut f, &mut visited, &mut vs) {
                    for u in vs {
                        f[u] = std::i64::MAX;
                    }
                }
