...
fn dfs(g: &Graph, a: &[usize], x: usize, v: usize, p: usize) -> usize {
    let s0 = g.children(v, p).map(|u| dfs(g, a, x, u, v)).sum::<usize>();

    let s1 = g
        .children(v, p)
        .map(|u| (a[u - 1] >= x) as usize)
        .sum::<usize>();

    (s0 + s1).saturating_sub(1)
}
...
fn main() {
    ...
    let ans = lower_bound_int(0, 1000000001, |x: usize| {
        if dfs(&g, &a, x, 0, n) >= 1 {
            Ordering::Less
        } else {
            Ordering::Greater
