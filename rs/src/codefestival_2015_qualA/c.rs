...
fn main() {
    ...
    let c = ab.citer().map(|(aa, bb)| aa - bb).collect_vec();

    let d = c.citer().sorted_by_key(|&cc| Reverse(cc)).collect_vec();
    let total_a = ab.citer().pick0().sum::<usize>();

    let ans = once(0)
        .chain(d.citer())
        .cumsum::<usize>()
        // .inspect(|tt| eprintln!("{}", tt))
        .find_position(|tt| t + tt >= total_a);
