...
fn main() {
    ...
    dfs(&g, 0, n, &c, &mut table);

    println!("{}", dfs2(&g, 0, n, &table, (0, 0)));
}

fn dfs(g: &Graph, v: usize, p: usize, c: &[usize], table: &mut [(usize, usize)]) {
    g.children(v, p).for_each(|u| {
        dfs(g, u, v, c, table);
    });
    table[v].0 = g
        .children(v, p)
        .map(|u| table[u].0 + table[u].1)
        .sum::<usize>();
    table[v].1 = c[v] + g.children(v, p).map(|u| table[u].1).sum::<usize>();
}

fn dfs2(g: &Graph, v: usize, p: usize, table: &[(usize, usize)], q: (usize, usize)) -> usize {
    let s = table[v].0 + q.0 + q.1;
    ...
    g.children(v, p)
        .map(|u| {
            dfs2(
                ...
                v,
                table,
                (s - table[u].0 - table[u].1, table[v].1 + q.1 - table[u].1),
            )
        })
        .chain(once(s))
        .min()
