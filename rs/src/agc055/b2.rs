...
fn main() {
    ...
    let to_idx = |s: &[usize]| -> usize { s.citer().fold(0, |x, c| 3 * x + c) };
            ...
            match &term[i..i + 3] {
                &[0, 1, 2] | &[1, 2, 0] | &[2, 0, 1] => {
                    ...
                    for j in 0..3 {
                        ...
                        uf.unite(to_idx(term), to_idx(&new_term));
                    }
                }
                _ => {}
            }
    ...
    terms
        .iter()
        .map(|term| (term, uf.root(to_idx(term))))
        .sorted_by_key(|(_term, ridx)| *ridx)
        .group_by(|(_term, ridx)| *ridx)
        .into_iter()
        .for_each(|(_idx, it)| {
            let mut delim = "";
            let terms_in_group = it.collect::<Vec<_>>();

            if terms_in_group.len() > 1 {
                for (term, _) in terms_in_group {
                    print!(
                        "{}{}",
                        delim,
                        term.citer().map(|c| (b'A' + c as u8) as char).join(""),
                        // to_idx(term),
                    );
                    delim = ", ";
                }
