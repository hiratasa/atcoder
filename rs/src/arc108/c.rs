...
fn dfs(
    ...
    let l = g.out_edges[v]
        ...
        .map(|e| {
            dfs(
                ...
                labels,
                e.label,
                !match_parent || parent_edge_label != e.label,
            )
        })
        .chain(once(parent_edge_label))
        .sorted()
        .dedup()
    ...
    labels[v] = if match_parent { parent_edge_label } else { l };
...
fn main() {
    ...
    let mut ans = vec![0; n];
    dfs(&g, 0, &mut vec![false; n], &mut ans, usize::MAX, false);
