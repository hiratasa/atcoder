    ...
    for _ in 0..q {
        ...
        if t == 1 {
            ...
            *s.entry(x).or_insert(0) += 1;
            ...
            let y = s.get(&x).copied().unwrap_or(0usize);
            s.insert(x, y.saturating_sub(c));
        } else {
            while let Some((&x, &y)) = s.iter().next() {
                if y > 0 {
                    break;
                }
            ...
            while let Some((&x, &y)) = s.iter().next_back() {
                if y > 0 {
                    break;
                }
            ...
            let xmax = s.keys().next_back().unwrap();
            let xmin = s.keys().next().unwrap();
