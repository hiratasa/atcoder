...
fn main() {
    ...
    let (table, _) = query.iter().enumerate().fold(
        (vec![vec![]; q], vec![None; n]),
        |(mut table, mut updated), (i_query, s)| {
            let mut it = s.split_whitespace();

            let t = it.next().unwrap().parse::<usize>().unwrap();

            if t == 2 {
                let i = it.next().unwrap().parse::<usize>().unwrap();
                let i = i - 1;
                updated[i] = Some(i_query);
            } else if t == 3 {
                let i = it.next().unwrap().parse::<usize>().unwrap();
                let i = i - 1;
                let j = it.next().unwrap().parse::<usize>().unwrap();
                let j = j - 1;
                if let Some(j_query) = updated[i] {
                    table[j_query].push((i_query, j));
                }
            }
    ...
    query
        ...
        .enumerate()
        .scan(
            (BIT::<Sum>::new(m + 1), vec![0i64; q]),
            |(bit, offsets), (i_query, s)| {
                let mut it = s.split_whitespace();
                ...
                if t == 1 {
                    ...
                    for &(j_query, j) in &table[i_query] {
                        offsets[j_query] = x - bit.sum(j + 1);
                    }
                    ...
                    Some(Some(bit.sum(j + 1) + offsets[i_query]))
                } else {
                    unreachable!()
                }
            },
