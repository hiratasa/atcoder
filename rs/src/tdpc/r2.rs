...
fn scc(g: &Graph) -> (Vec<Vec<usize>>, Vec<usize>, Vec<Vec<usize>>) {
    ...
    let mut ret = vec![];
    let mut idxs = vec![usize::MAX; g.size()];
    let mut scc_edges = vec![];
    {
        for &v in vs.iter().rev() {
            if idxs[v] == usize::MAX {
                ...
                ret.push(component);
                scc_edges.push(vec![]);
                for idx in adjs.citer().sorted().dedup() {
...
fn main() {
    let n = read::<usize>();
    let adjs = read_mat::<usize>(n);

    let g = Graph::from_edges_directed(
        n,
        (0..n).flat_map(|i| adjs[i].citer().positions(|x| x > 0).map(move |j| (i, j))),
    );

    let (components, _idxs, edges) = scc(&g);

    let m = components.len();
    let dp = (0..m)
        .rev()
        .flat_map(|i| (i..m).rev().map(move |j| (i, j)))
                    ...
                    .map_or_else(
                        || {
                            edges[j]
                                ...
                                .map(|k| dp[i][k])
                                .max()
                                .unwrap_or(components[i].len())
                                + components[j].len()
                        },
                        |x| x + components[i].len(),
                    )
            } else {
                edges[i]
                    .citer()
    ...
    println!(
        "{}",
        dp.iter().map(|r| r.citer().max().unwrap()).max().unwrap()
    );
}
