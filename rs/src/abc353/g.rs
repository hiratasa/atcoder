...
fn main() {
    ...
    let mut st = SegmentTree::<Maximum>::new(n);
    st.set(0, 0);

    let (ans, _, _) = tp.into_iter().fold(
        (0, st.clone(), st.clone()),
        |(ans, mut st_neg, mut st_pos), (t, p)| {
            // p - c * (t - i) + dp[i]
            let x_left = st_pos.query(..t).0.saturating_add(p - c * t as i64);

            // p - c * (i - t) + dp[i]
            let x_right = st_neg.query(t..).0.saturating_add(p + c * t as i64);

            let x = max(x_left, x_right);

            let ans = max(ans, x);

            st_neg.set(t, max(st_neg.get(t).0, x - c * t as i64));
            st_pos.set(t, max(st_pos.get(t).0, x + c * t as i64));
...
define_monoid!(Maximum, i64, i64::MIN, std::cmp::max);
