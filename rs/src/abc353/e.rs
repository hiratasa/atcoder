...
fn main() {
    ...
    let t = s
        .iter()
        .tuple_windows()
        .map(|(s0, s1)| izip!(s0, s1).take_while(|&(c0, c1)| c0 == c1).count())
        .collect::<Vec<_>>();

    let st = SegmentTree::<Minimum>::with(&t);
    ...
    let ans = (0..n)
        .map(|i| {
            ...
            let sum_l = (1..=l)
                .map(|ll| {
                    i - st
                        .left_partition_point(i, |&Minimum(c)| c >= ll)
                        .unwrap_or(i)
                })
                .sum::<usize>();
            ...
            // let sum_r = (1..=l)
            //     .map(|ll| {
            //         st.right_partition_point(i, |&Minimum(c)| c >= ll)
            //             .unwrap_or(n)
            //             - (i + 1)
            //     })
            //     .sum::<usize>();
...
define_monoid!(Minimum, usize, usize::MAX, std::cmp::min);
