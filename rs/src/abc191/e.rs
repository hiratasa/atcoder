...
use detail::WeightedGraph as Graph;
...
fn main() {
    ...
    let costs = (0..n).map(|src| dijkstra(&g, src)).collect_vec();

    for i in 0..n {
        if let Some(cost) = g.out_edges[i]
            .citer()
            .map(|edge| costs[edge.to][i].saturating_add(edge.label))
