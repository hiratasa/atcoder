fn main() {
    ...
    let mut st = SegmentTree::<Sum>::new(M + 1);
    let mut nums = SegmentTree::<Sum>::new(M + 1);
    for &x in &a {
        st.set(x, st.get(x).0 + x);
        nums.set(x, nums.get(x).0 + 1);
    }
    (0..q)
        .scan((a, st, nums), |(a, st, nums), _| {
            input! {
                ty: usize,
            };
            ...
            if ty == 1 {
                ...
                st.set(a[x], st.get(a[x]).0 - a[x]);
                nums.set(a[x], nums.get(a[x]).0 - 1);
                a[x] = y;
                st.set(a[x], st.get(a[x]).0 + a[x]);
                nums.set(a[x], nums.get(a[x]).0 + 1);
                ...
                if l > r {
                    ...
                    let lnum = nums.query(0..l).0;
                    let rnum = nums.query(r + 1..).0;
                    let sum = st.query(l..=r).0;

                    Some(Some(lnum * l + rnum * r + sum))
                }
            }
        })
        .flatten()
