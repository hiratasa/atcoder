...
fn dfs(g: &Graph, visited: &mut [usize], v: usize) -> (usize, usize) {
    if visited[v] == 1 {
        // 別のDFSで訪問済み
        return (0, usize::MAX);
    }
    ...
    let e = g.out_edges[v][0];

    visited[v] = 2;
    let (k, q) = dfs(g, visited, e.to);
    visited[v] = 1;

    let k2 = if q != usize::MAX { min(k, e.label) } else { k };
    let q2 = if q == v { usize::MAX } else { q };
...
fn main() {
    ...
    let g = Graph::from_edges_directed(
        n,
        izip!(x, c).enumerate().map(|(i, (xx, cc))| (i, xx - 1, cc)),
    );

    let ans = (0..n)
        .scan(vec![0; n], |visited, i| Some(dfs(&g, visited, i).0))
        .sum::<usize>();
