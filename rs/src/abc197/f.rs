...
fn main() {
    ...
    let g = Graph::from_edges_undirected(
        n * n,
        iproduct!(abc.citer(), abc.citer())
            .filter(|(t0, t1)| t0.2 == t1.2)
            .flat_map(|(t0, t1)| {
                it![
                    ((t0.0 - 1) * n + t1.0 - 1, (t0.1 - 1) * n + t1.1 - 1),
                    ((t0.0 - 1) * n + t1.1 - 1, (t0.1 - 1) * n + t1.0 - 1)
                ]
            }),
    );
    ...
    let adj = abc
        .citer()
        .fold(vec![vec![false; n]; n], |mut adj, (a, b, _)| {
            adj[a - 1][b - 1] = true;
            adj[b - 1][a - 1] = true;
            adj
        });

    let mut costs = vec![1usize << 60; n * n];
    ...
    while let Some(v) = q.pop_front() {
        ...
        if adj[a][b] {
            ans = min(ans, cost * 2 + 1);
        }

        g.out_edges[v]
            .citer()
            .filter(|edge| chmin(&mut costs[edge.to], cost + 1))
            .for_each(|edge| q.push_back(edge.to));
    }
