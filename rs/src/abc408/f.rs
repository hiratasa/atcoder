fn main() {
    ...
    let (_, dp) = iproduct!(0..n, 0..2)
        .sorted_by_key(|&(i, j)| (h[i] + d * j, Reverse(j)))
        .rev()
        .fold(
            (SegmentTree::<Max>::new(n), vec![0; n]),
            |(mut st, mut dp), (i, j)| {
                if j == 0 {
                    st.set(i, dp[i]);
                } else {
                    dp[i] = st.query(i.saturating_sub(r)..=min(n - 1, i + r)).0 + 1;
                }
    ...
    let ans = dp.into_iter().max().unwrap() - 1;
