fn main() {
    ...
    let sum = abc.iter().map(|e| e.2).sum::<usize>();

    let g = Graph::from_edges_undirected(n, abc);

    let (v, _) = find_farthest(&g, 0, n);
    let (_, d) = find_farthest(&g, v, n);
...
fn find_farthest(g: &Graph<usize>, v: usize, p: usize) -> (usize, usize) {
    g.children_edge(v, p)
        .map(|e| {
            let (x, d) = find_farthest(g, e.to, v);
            (x, d + e.label)
        })
        .max_by_key(|&(_, d)| d)
        .unwrap_or((v, 0))
