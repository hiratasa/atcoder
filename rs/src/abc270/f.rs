...
fn main() {
    ...
    let y = read_row::<usize>();
    let abz = read_vec(m, || read_tuple!(usize, usize, usize));

    let edges = x
        .citer()
        .enumerate()
        .map(|(i, x)| (x, i, n))
        .chain(y.citer().enumerate().map(|(i, y)| (y, i, n + 1)))
        .chain(abz.citer().map(|(a, b, z)| (z, a - 1, b - 1)))
        .sorted()
        .collect::<Vec<_>>();

    let ans = iproduct!([false, true].citer(), [false, true].citer())
        .map(|(use_n, use_n1)| {
            let (cost, mut uf) =
                edges
                    .citer()
                    .fold((0, UnionFind::new(n + 2)), |(cost, mut uf), (z, a, b)| {
                        if !use_n && b == n {
                            return (cost, uf);
                        }
            ...
            if uf.size(0) - uf.same(0, n) as usize - uf.same(0, n + 1) as usize == n {
                cost
            } else {
                usize::MAX
            }
