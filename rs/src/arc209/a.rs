fn main() {
    ...
    sk.into_iter()
        .map(|(s, k)| {
            ...
            let t = once(0i64)
                .chain(s.iter().copied().map(|c| if c == '(' { 1 } else { -1 }))
                .cumsum::<i64>()
                .collect::<Vec<_>>();

            if !(t[n] == 0 && t.iter().all(|&x| x >= 0)) {
                return true;
            }

            let from_left = (0..n).fold(vec![0; n], |mut a, i| {
                if i >= 1 && s[i - 1] == '(' && s[i] == ')' {
                    if i == 1 {
                        a[i] = 1;
                    } else {
            ...
            let from_right = (0..n).rev().fold(vec![0; n], |mut a, i| {
                if i < n - 1 && s[i] == '(' && s[i + 1] == ')' {
                    if i == n - 2 {
                        a[i] = 1;
                    } else {
            ...
            let mut l = 0;
            let mut r = n;
            let st = SegmentTree::<SumMin>::with(
                &s.iter()
                    .map(|&c| if c == '(' { 1 } else { -1 })
                    .map(|x| (x, min(0, x)))
                    .collect::<Vec<_>>(),
            ...
            while r - l > k {
                if from_right[l] > 0 && from_left[r - 1] > 0 {
                    if from_right[l] > from_left[r - 1] {
                        r -= 2;
                    } else {
                        l += 2;
                    }
                ...
                assert_eq!(st.query(l..r).0.0, 0);
                if st.query(l..r).0.1 < 0 {
                    return true;
                }
            }
...
define_monoid!(
    ...
    (i64, i64),
    (0, i64::MAX),
    |(a, b): (i64, i64), (c, d): (i64, i64)| { (a + c, min(b, a.saturating_add(d))) }
);
