...
mod scc {
    ...
    fn rev_dfs(
        ...
        for u in g.in_edges[v].citer().map(|e| e.from) {
            let idx2 = idxs[u];
            if idx2 == std::usize::MAX {
    ...
    pub fn scc(g: &Graph) -> (Vec<Vec<usize>>, Vec<usize>, Graph) {
        let mut vs = vec![];
        {
        ...
        (
            components,
            idxs,
            Graph::from_edges_directed(k, edges.citer().sorted().dedup()),
        )
    }
}
...
fn main() {
    ...
    let g = Graph::from_edges1_directed(n, uv);

    let (components, _idxs, g1) = scc(&g);

    let k = components.len();
    let mut dp = vec![false; k];
    let mut ans = 0;
    for i in (0..k).rev() {
        dp[i] = components[i].len() > 1 || g1.adjs(i).any(|j| dp[j]);
        if dp[i] {
            ans += components[i].len();
        }
    }
