...
fn main() {
    ...
    let a = read_vec(r, || read_row::<usize>());
    let b = read_vec(r - 1, || read_row::<usize>());

    let mut g = Graph::new(2 * r * c);

    for i in 0..r {
        for j in 0..c - 1 {
            g.add_edge(Edge::from((i * c + j, i * c + j + 1, a[i][j])));
            g.add_edge(Edge::from((i * c + j + 1, i * c + j, a[i][j])));
        }
    }

    for i in 0..r - 1 {
        for j in 0..c {
            g.add_edge(Edge::from((i * c + j, (i + 1) * c + j, b[i][j])));
        }
    }

    for i in 0..r {
        for j in 0..c {
            g.add_edge(Edge::from((i * c + j, r * c + i * c + j, 1)));
            g.add_edge(Edge::from((r * c + i * c + j, i * c + j, 0)));
        }
    }

    for i in 1..r {
        for j in 0..c {
            g.add_edge(Edge::from((r * c + i * c + j, r * c + (i - 1) * c + j, 1)));
        }
    }

    let ans = dijkstra1(&g, 0, r * c - 1).unwrap();
    println!("{}", ans);
}
