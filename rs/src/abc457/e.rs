fn main() {
    ...
    let intervals = lr
        ...
        .chunk_by(|t| t.0)
        .into_iter()
        .map(|(l, it)| (l, it.map(|t| t.1).collect::<Vec<_>>()))
        .fold(vec![vec![]; n], |mut t, (l, v)| {
            t[l] = v;
            t
        });
    let rev_intervals = lr
        .iter()
        .copied()
        .map(|(l, r)| (r, l))
        .sorted()
    ...
    let st = SegmentTree::<Min>::with(
        &intervals
            .iter()
            .map(|x| x.first().copied().unwrap_or(usize::MAX))
            .collect::<Vec<_>>(),
    );
    ...
    queries
        ...
        .map(|(s, t)| {
            let idx = intervals[s]
                .binary_search_by(|&r| r.cmp(&t).then(Ordering::Less))
                .unwrap_err();
            if idx == 0 {
                return false;
            }

            let r = intervals[s][idx - 1];

            if r == t {
                idx > 1 || st.query(s + 1..t).0 <= t
            } else {
                let idx2 = rev_intervals[t]
                    .binary_search_by(|&l| l.cmp(&s).then(Ordering::Greater))
                    .unwrap_err();
                if idx2 >= rev_intervals[t].len() {
                    false
                } else {
                    let l = rev_intervals[t][idx2];
