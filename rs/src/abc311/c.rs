...
fn find_cycle(g: &Graph, v: usize, visited: &mut [bool]) -> Option<(Vec<usize>, usize)> {
    if visited[v] {
        return Some((vec![v], v));
    }
    ...
    g.adjs(v)
        .find_map(|u| find_cycle(g, u, visited))
        .map(|(path, x)| {
            if x == usize::MAX || x == v {
                (path, usize::MAX)
            } else {
...
fn main() {
    ...
    let g = Graph::from_edges_directed(n, a.citer().enumerate().map(|(i, x)| (i, x - 1)));

    let (path, _) = find_cycle(&g, 0, &mut vec![false; n]).unwrap();

    println!("{}", path.len());
    println!("{}", path.citer().rev().map(|i| i + 1).join(" "));
}
