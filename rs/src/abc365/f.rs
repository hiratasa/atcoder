fn main() {
    ...
    let st_l = SegmentTree::<Max>::with(&lr.iter().map(|v| v[0]).collect::<Vec<_>>());
    let st_r = SegmentTree::<Min>::with(&lr.iter().map(|v| v[1]).collect::<Vec<_>>());

    let mut t0 = (0..n)
        .map(|x| {
            core::array::from_fn::<_, 2, _>(|i| {
                min(
                    st_l.right_partition_point(x, |&Max(y)| y <= lr[x][i])
                        .map_or((n, 0, 0), |xx| (xx - 1, 0, lr[xx - 1][0] - lr[x][i])),
                    st_r.right_partition_point(x, |&Min(y)| y >= lr[x][i])
                        .map_or((n, 1, 0), |xx| (xx - 1, 1, lr[x][i] - lr[xx - 1][1])),
                )
            })
        })
        .collect::<Vec<_>>();
    ...
    let mut t = vec![vec![]; 31];
    t[0] = t0;
    for i in 0..30 {
        t[i + 1].resize_with(n + 1, Default::default);
        for j in 0..n {
            for k in 0..2 {
                let (x, l, c) = t[i][j][k];
                let (x2, l2, c2) = t[i][x][l];
                t[i + 1][j][k] = (x2, l2, c + c2);
            }
        }
        t[i + 1][n] = [(n, 0, 0); 2];
    }
    ...
    for query in queries {
        ...
        let (x, i) = min(
            st_l.right_partition_point(sx, |&Max(y)| y <= sy)
                .map_or((n, 0), |xx| (xx - 1, 0)),
            st_r.right_partition_point(sx, |&Min(y)| y >= sy)
                .map_or((n, 1), |xx| (xx - 1, 1)),
        );
