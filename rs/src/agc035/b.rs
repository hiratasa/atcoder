...
fn dfs(
    ...
    p: usize,
    state: &mut [usize],
    directed_edges: &mut Vec<(usize, usize)>,
) -> bool {
    state[v] = 1;
    ...
    let d = g.out_edges[v]
        ...
        .flat_map(|edge| {
            if state[edge.to] == 1 {
                directed_edges.push((v, edge.to));
                return Some(true);
            } else if state[edge.to] == 2 {
                return None;
            }

            let from_v = dfs(g, edge.to, v, state, directed_edges);
            if from_v {
                directed_edges.push((v, edge.to));
            } else {
                directed_edges.push((edge.to, v));
            }
            Some(from_v)
        })
        .map_into::<usize>()
...
fn main() {
    ...
    dfs(&g, 0, n, &mut vec![0; n], &mut directed_edges);
