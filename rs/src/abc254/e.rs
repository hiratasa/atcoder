...
fn dfs(g: &Graph, v: usize, visited: &mut FxHashSet<usize>, k: usize) {
    visited.insert(v);
    ...
    g.adjs(v).for_each(|u| dfs(g, u, visited, k - 1));
}
...
fn main() {
    ...
    let q = read::<usize>();
    let xk = read_vec(q, || read_tuple!(usize, usize));
    ...
    xk.citer()
        .map(|(x, k)| {
            ...
            dfs(&g, x - 1, &mut visited, k);

            visited.into_iter().map(|y| y + 1).sum::<usize>()
        })
