...
fn dfs(g: &Graph, v: usize, p: usize, s: &[char], colors: &mut [Option<char>]) -> bool {
    if !g.children(v, p).all(|u| dfs(g, u, v, s, colors)) {
        return false;
    }
    ...
    if same > diff + (p < g.size()) as usize {
        // 親の色に関わらず条件を満たす
        true
    } else if same + (p < g.size()) as usize > diff {
        // 親が同色なら条件を満たす
        if colors[p].is_none() {
            colors[p] = Some(s[v]);
            true
        } else if colors[p] == Some(s[v]) {
            true
        } else {
            false
        }
...
fn main() {
    ...
    for _ in 0..t {
        ...
        let ok = dfs(&g, 0, n, &s, &mut colors);

        if ok {
            println!("{}", colors.citer().map(|c| c.unwrap_or('B')).join(""));
        } else {
            println!("-1");
        }
    }
