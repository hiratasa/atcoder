...
fn calc(g: &Graph, v: usize, p: usize, sizes: &[usize], x: &mut usize, y: &mut usize) {
    let s = g.size() - sizes[v];
...
fn main() {
    ...
    let mut x = 0;
    let mut y = 0;
    calc(&g, 0, n, &sizes, &mut x, &mut y);

    let ans = ((n - 1).pow(3) * n - x - 3 * y) / 6;
