...
fn main() {
    ...
    let uvw = read_vec(q, || read_tuple!(usize, usize, usize));
    ...
    let ans = chain(
        abc.citer().map(|(a, b, c)| Edge(a - 1, b - 1, c)),
        uvw.citer()
            .enumerate()
            .map(|(i, (u, v, w))| Query(i, u - 1, v - 1, w)),
    )
    ...
    .scan(UnionFind::new(n), |uf, item| match item {
        ...
        Query(i_query, u, v, _) => Some(Some((i_query, !uf.same(u, v)))),
    })
    .flatten()
    .fold(vec![false; q], |mut ans, (i_query, a)| {
        ans[i_query] = a;
        ans
    });
