...
fn find_centroid(g: &Graph, v: usize, p: usize, sizes: &mut [usize]) -> Option<usize> {
    let n = g.size();
    sizes[v] = 1;
    g.children(v, p)
        .find_map(|u| {
...
fn dfs(g: &Graph, v: usize, p: usize, idxs: &mut Vec<usize>) {
    idxs.push(v);

    g.children(v, p).for_each(|u| dfs(g, u, v, idxs))
}
...
fn main() {
    ...
    let c = find_centroid(&g, 0, n, &mut sizes).unwrap();

    let mut ans = vec![0; n];

    if 2 * sizes[c] == n {
        let p = g.adjs(c).find(|&v| sizes[v] > sizes[c]).unwrap();
