...
#[allow(dead_code)]
fn dfs(g: &Graph, v: usize, p: usize, dist: &mut [usize]) -> (usize, usize) {
    g.out_edges[v]
        .iter()
        .filter(|&e| e.to != p)
        .map(|e| {
            dist[e.to] = dist[v] + e.label;
            dfs(g, e.to, v, dist)
        })
        .max()
        .unwrap_or((dist[v], v))
...
fn main() {
    ...
    let g = Graph::from_edges1_undirected(
        2 * n,
        abc.citer().chain((1..=n).map(|i| (i, n + i, d[i - 1]))),
    );

    let (_, v) = dfs(&g, 0, 2 * n, &mut vec![0; 2 * n]);
    let mut dist0 = vec![0; 2 * n];
    let (_, u) = dfs(&g, v, 2 * n, &mut dist0);
    let mut dist1 = vec![0; 2 * n];
    let (_, _) = dfs(&g, u, 2 * n, &mut dist1);
