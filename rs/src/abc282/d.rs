...
fn dfs(g: &Graph, v: usize, visited: &mut [bool], colors: &mut [bool]) -> Option<(usize, usize)> {
    visited[v] = true;

    g.adjs(v).try_fold((1, 0), |(num0, num1), u| {
        if visited[u] {
            ...
            let (k0, k1) = dfs(g, u, visited, colors)?;
            Some((num0 + k1, num1 + k0))
        }
    })
}
...
fn main() {
    ...
    if let Some((ans, _, _)) = (0..n).try_fold(
        (0, vec![false; n], vec![false; n]),
        |(t, mut visited, mut colors), i| {
            if visited[i] {
                Some((t, visited, colors))
            } else {
                let (num0, num1) = dfs(&g, i, &mut visited, &mut colors)?;

                Some((
                    t + 2 * num0 * num1 + (num0 + num1) * (n - (num0 + num1)),
                    visited,
                    colors,
                ))
            }
