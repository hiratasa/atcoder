...
fn calc(
    ...
    g.children_edge(v, p).for_each(|edge| {
        let u = edge.to;
        let (w0, w1) = (edges[edge.label].2, edges[edge.label].3);
        weights[u][0] = weights[v][0] ^ w0;
        weights[u][1] = weights[v][1] ^ w1;

        calc(g, u, v, edges, weights)
    });
}
...
fn main() {
    ...
    let edges = read_vec(n - 1, || read_tuple!(usize, usize, usize, usize));
    ...
    let g = Graph::from_edges1_undirected(
        ...
        edges
            .citer()
            .enumerate()
            .map(|(i, (from, to, _, _))| (from, to, i)),
    );

    let mut weights = vec![[0; 2]; n];
    calc(&g, 0, n, &edges, &mut weights);

    let x = weights.citer().fold(0, |x, [y, z]| x ^ y ^ z);

    if weights
        .citer()
        .map(|w| w[0] ^ x)
        .sorted()
        .eq(weights.citer().map(|w| w[1]).sorted())
    {
        println!("YES");
    } else {
        println!("NO");
