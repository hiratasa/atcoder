...
fn main() {
    ...
    let calc_inversion = |a: &[usize]| {
        if a.is_empty() {
            return 0;
        }

        let m = a.len();
        let idxs = (0..m).sorted_by_key(|&i| a[i]).collect::<Vec<_>>();

        idxs.citer()
            .scan(BIT::<Sum>::new(m), |bit, idx| {
                bit.add(idx, 1);
                Some(bit.sum_between(idx, m))
            })
            .sum::<usize>()
    };

    let t0 = calc_inversion(&x);

    let t1 = izip!(c, x)
        .fold(vec![vec![]; n + 1], |mut t, (cc, xx)| {
            t[cc].push(xx);
            t
        })
        .into_iter()
        .map(|t| calc_inversion(&t))
        .sum::<usize>();
