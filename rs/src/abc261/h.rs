...
fn main() {
    let (n, m, v) = read_tuple!(usize, usize, usize);
    let abc = read_vec(m, || read_tuple!(usize, usize, usize));

    let g = Graph::from_edges1_directed(n, abc);

    let mut weights = vec![vec![usize::MAX; n], vec![0; n]];
    let mut counts = vec![0; n];
    let mut q = BinaryHeap::new();

    for i in (0..n).filter(|&i| g.out_edges[i].len() == 0) {
        weights[0][i] = 0;
        weights[1][i] = 0;
        q.push((Reverse(0), 0, i));
        q.push((Reverse(0), 1, i));
    }

    while let Some((Reverse(w), i, u)) = q.pop() {
        if w != weights[i][u] {
            continue;
        }

        if i == 0 {
            g.in_edges[u].citer().for_each(|e| {
                weights[1][e.from] = max(weights[1][e.from], w + e.label);
                counts[e.from] += 1;

                if counts[e.from] == g.out_edges[e.from].len() {
                    q.push((Reverse(weights[1][e.from]), 1, e.from));
                }
            });
        } else {
            g.in_edges[u].citer().for_each(|e| {
                if w + e.label < weights[0][e.from] {
                    weights[0][e.from] = w + e.label;
                    q.push((Reverse(weights[0][e.from]), 0, e.from));
                }
            });
        }
    }

    if weights[0][v - 1] == usize::MAX {
        println!("INFINITY");
    } else {
        println!("{}", weights[0][v - 1]);
    }
}
