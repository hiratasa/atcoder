fn main() {
    ...
    let g = Graph::from_edges_directed(n, xy);

    queries
        .into_iter()
        .scan(vec![false; n], |visited, (t, v)| {
            if t == 1 {
                dfs_rev(&g, v, visited);
...
fn dfs_rev(g: &Graph, v: usize, visited: &mut [bool]) {
    if visited[v] {
        return;
    }
    visited[v] = true;

    g.in_edges[v].iter().for_each(|&e| {
        dfs_rev(g, e.from, visited);
    });
}
