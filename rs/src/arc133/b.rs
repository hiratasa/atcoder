...
impl Operator<usize> for Maximum {
...
fn main() {
    ...
    let mut st = (0..n).fold(
        LazySegmentTree::<Maximum, Maximum>::new(n + 1),
        |mut st, i| {
            let tmp = positions[i]
                .citer()
                .map(|pos| (pos, st.get(pos) + 1))
                .collect::<Vec<_>>();

            izip!(tmp, positions[i].citer().skip(1).chain(once(n))).fold(
                st,
                |mut st, ((pos0, v), pos1)| {
                    st.update(pos0 + 1, pos1 + 1, v);
                    st
                },
            )
        },
