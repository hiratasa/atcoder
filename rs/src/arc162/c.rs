...
fn dfs(g: &Graph, v: usize, a: &[Option<usize>], exists: &mut [bool]) -> usize {
    if let Some(x) = a[v] {
        exists[x] = true;
    }

    g.adjs(v).map(|u| dfs(g, u, a, exists)).sum::<usize>() + a[v].is_none() as usize
}

fn solve0(g: &Graph, k: usize, a: &mut [Option<usize>], alice: bool) -> bool {
    let n = a.len();

    if (0..n).any(|i| {
        let mut exists = vec![false; n + 1];
        let c = dfs(g, i, &a, &mut exists);

        c == 0 && (0..k).all(|i| exists[i]) && !exists[k]
    }) {
        return true;
    }

    let idxs = a.citer().positions(|x| x.is_none()).collect::<Vec<_>>();
    ...
    if alice {
        idxs.citer().any(|idx| {
            (0..=n).any(|x| {
                a[idx] = Some(x);
                let res = solve0(&g, k, a, !alice);
                a[idx] = None;

                if res {
                    eprintln!("Alice={}, idx={}, x={}", alice, idx, x);
                }
...
fn main() {
    let t = read::<usize>();
    let cases = read_vec(t, || {
        let (n, k) = read_tuple!(usize, usize);
        let p = read_row::<usize>();
        let a = read_row::<i64>()
            .into_iter()
            .map(|x| usize::try_from(x).ok())
            .collect::<Vec<_>>();
    ...
    cases
        ...
        .map(|(k, p, a)| {
            ...
            let g =
                Graph::from_edges_directed(n, p.citer().enumerate().map(|(i, j)| (j - 1, i + 1)));
            ...
            let ans = (0..n).any(|i| {
                ...
                (c == 0 && !exists[k] && (0..k).all(|l| exists[l]))
                    || (c == 1 && !exists[k] && (0..k).filter(|&l| !exists[l]).count() <= 1)
            });

            // let mut a = a;
            // let ans0 = solve0(&g, k, &mut a, true);
            // assert_eq!(
            //     ans,
            //     ans0,
            //     "{} {}\n{}\n{}",
            ...
            //     k,
            //     p.citer().join(" "),
            //     a.citer().map(|x| x.map_or(-1, |x| x as i64)).join(" ")
            // );
