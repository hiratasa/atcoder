...
fn mul(a: &[[i64; 8]; 8], b: &[[i64; 8]; 8]) -> [[i64; 8]; 8] {
    let mut c = [[i64::MIN; 8]; 8];

    for i in 0..8 {
        for j in 0..8 {
            for k in 0..8 {
                c[i][j] = max(c[i][j], a[i][k].saturating_add(b[k][j]));
            }
        }
    }
...
fn main() {
    ...
    let calc_mat = |i: usize, a: &[i64]| {
        let mut mat = [[i64::MIN; 8]; 8];

        (0..8).for_each(|s| {
            let picked = s & 1 > 0;
            let prev2_is_right = s & 2 > 0;
            let prev_is_right = s & 4 > 0;

            it![false, true].for_each(|is_right| {
                let mut p = 0;
                if i >= 2 && prev2_is_right {
                    p += a[i - 2]
                };
                if i < n && !is_right {
                    p += a[i];
                ...
                let (next_pick, q) = match (picked, p > 0) {
                    (false, false) => (false, 0),
                    (false, true) => (true, p),
                    (true, false) => (true, 0),
                    (true, true) => (false, 0),
                ...
                let next_s = (next_pick as usize)
                    + ((prev_is_right as usize) << 1)
                    + ((is_right as usize) << 2);

                mat[s][next_s] = max(mat[s][next_s], q);
            });
        });
    ...
    let t = (0..=n + 1).map(|i| calc_mat(i, &a)).collect::<Vec<_>>();
    let mut st = SegmentTree::<MatMul>::with(&t);

    let q = read::<usize>();
    for _ in 0..q {
        let (k, x) = read_tuple!(usize, i64);
        let k = k - 1;

        a[k] = x;

        st.set(k, calc_mat(k, &a));
        st.set(k + 2, calc_mat(k + 2, &a));

        let mat = st.query(0, n + 2).0;
        let ans = mat[0].citer().max().unwrap();
