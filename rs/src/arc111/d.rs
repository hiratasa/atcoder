...
fn dfs(g: &Graph, v: usize, c: &[usize], visited: &mut [bool], dir: &mut [Option<(usize, usize)>]) {
    if visited[v] {
        return;
    }
    ...
    g.out_edges[v].citer().for_each(|Edge { to, label, .. }| {
        if dir[label].is_none() {
            if c[v] < c[to] {
                dir[label] = Some((to, v));
            } else {
                dir[label] = Some((v, to));
                dfs(g, to, c, visited, dir);
            }
        }
    });
}
...
fn main() {
    ...
    for v in (0..n).sorted_by_key(|&i| Reverse(c[i])) {
        dfs(&g, v, &c, &mut visited, &mut dir);
    }

    for (i, x) in dir.citer().enumerate() {
        let (a, b) = x.unwrap();
        if (a + 1, b + 1) == ab[i] {
            println!("->");
        } else {
