...
fn main() {
    ...
    let s = read_str();
    let q = read::<usize>();
    let query = read_vec(q, || read_tuple!(usize, usize, String));
    ...
    let mut init = vec![SegmentTree::<Sum>::new(n); 26];
    for (i, c) in s.citer().enumerate() {
        init[c as usize - 'a' as usize].set(i, 1);
    }

    query
        .iter()
        .scan((s, t, init), |(s, t, sts), qq| {
            if qq.0 == 1 {
                let x = qq.1 - 1;
                let c = qq.2.chars().next().unwrap();

                let left0 = x > 0 && s[x - 1] <= s[x];
                let right0 = x + 1 < n && s[x] <= s[x + 1];

                sts[s[x] as usize - 'a' as usize].set(x, 0);
                sts[c as usize - 'a' as usize].set(x, 1);
                s[x] = c;

                let left1 = x > 0 && s[x - 1] <= s[x];
                let right1 = x + 1 < n && s[x] <= s[x + 1];
                ...
                let l = qq.1 - 1;
                let r = qq.2.parse::<usize>().unwrap();

                if t.range(l + 1..r).next().is_some() {
                    return Some(Some(false));
                }

                let freq = sts.iter().map(|st| st.query(..).0).collect::<Vec<_>>();
                let subfreq = sts.iter().map(|st| st.query(l..r).0).collect::<Vec<_>>();

                let c0 = s[l] as usize - 'a' as usize;

                let ok = once(subfreq[c0])
                    .chain(freq[c0 + 1..].citer())
                    .scan(0, |sum, x| {
                        *sum += x;

                        Some(min(x, (r - l).saturating_sub(*sum - x)))
                    })
                    .eq(subfreq[c0..].citer());
