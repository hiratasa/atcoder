...
fn main() {
    ...
    let ans = points
        ...
        .scan(
            (
                SegmentTree::<MinMax>::new(s.len()),
                SegmentTree::<MinMax>::new(s.len()),
            ),
            |(st_x, st_y), (_v, u, x, y)| {
                let idx = s[&u];

                st_x.set(idx, MinMax::op(&st_x.get(idx), &(x, x)));
                st_y.set(idx, MinMax::op(&st_y.get(idx), &(y, y)));

                let (y_min, y_max) = st_y.query(0, idx + 1);
                let (x_min, x_max) = st_x.query(idx, s.len());

                Some(max(
                    max((y - y_min).abs(), (y - y_max).abs()),
                    max((x - x_min).abs(), (x - x_max).abs()),
                ))
            },
        )
        .max()
