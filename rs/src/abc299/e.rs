...
fn main() {
    ...
    let k = read::<usize>();
    let pd = read_vec(k, || read_tuple!(usize, usize));

    let g = Graph::from_edges1_undirected(n, uv);

    let is_black = pd.citer().fold(vec![true; n], |mut is_black, (p, d)| {
        let p = p - 1;
        ...
        while let Some(v) = q.pop_front() {
            ...
            q.extend(g.adjs(v).filter(|&u| {
                if dists[v] + 1 < dists[u] {
                    dists[u] = dists[v] + 1;
                    true
                } else {
                    false
    ...
    if ok {
        println!("Yes");
        println!("{}", (0..n).map(|i| is_black[i] as usize).join(""));
    } else {
        println!("No");
    }
}
