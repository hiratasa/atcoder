...
fn find_farthest(g: &Graph, v: usize, p: usize) -> (usize, usize) {
    g.children(v, p)
        ...
        .max()
        .map_or((1, v), |(d, u)| (d + 1, u))
}

fn calc_parent(g: &Graph, v: usize, parents: &mut [usize]) {
    g.children(v, parents[v]).for_each(|u| {
        parents[u] = v;
        calc_parent(g, u, parents);
    });
}
...
fn main() {
    ...
    let q = read::<usize>();
    let uk = read_vec(q, || read_tuple!(usize, usize));

    let g = Graph::from_edges1_undirected(n, ab);

    let (_, r) = find_farthest(&g, 0, n);
    let (_, r2) = find_farthest(&g, r, n);

    let mut parents = vec![vec![r; n]; 30];
    let mut parents2 = vec![vec![r2; n]; 30];
    calc_parent(&g, r, &mut parents[0]);
    calc_parent(&g, r2, &mut parents2[0]);
    for i in 1..30 {
        for v in 0..n {
            parents[i][v] = parents[i - 1][parents[i - 1][v]];
            parents2[i][v] = parents2[i - 1][parents2[i - 1][v]];
        }
    }

    let find_kth_parent = |v: usize, k: usize, t: usize| {
        let parents = if t == 0 { &parents } else { &parents2 };
    ...
    uk.citer()
        .map(|(u, k)| {
            ...
            find_kth_parent(u, k, 0).or(find_kth_parent(u, k, 1))
        })
        .for_each(|ans| {
            if let Some(ans) = ans {
                println!("{}", ans + 1);
