...
fn main() {
    ...
    let ans = lower_bound_int(0, 1usize << 60, |c| {
        let mut costs = (0..n)
            .map(|i| g.adjs(i).map(|j| a[j]).sum::<usize>())
            .collect::<Vec<_>>();

        let mut q = costs.citer().positions(|x| x <= c).collect::<Vec<_>>();

        let mut num_removed = 0;
        while let Some(i) = q.pop() {
            num_removed += 1;
            g.adjs(i).for_each(|j| costs[j] -= a[i]);

            q.extend(g.adjs(i).filter(|&j| costs[j] <= c && costs[j] + a[i] > c));
        }
