...
fn main() {
    ...
    let g = Graph::from_edges_directed(n, &uvw);
    ...
    for i in 0..n {
        for j in 0..n {
            for k in 0..n {
                dists[j][k] = min(dists[j][k], dists[j][i].saturating_add(dists[i][k]));
            }
        }
    }

    let dp = iproduct!(0..1 << n, 0..n).fold(vec![vec![i64::MAX; n]; 1 << n], |mut dp, (s, i)| {
        if s & (1 << i) == 0 {
            return dp;
        }
        ...
        dp[s][i] = if s == 1 << i {
            ...
            (0..n)
                ...
                .map(|j| dp[s ^ (1 << i)][j].saturating_add(dists[j][i]))
                .min()
                .unwrap()
        };
    ...
    let ans = dp[(1 << n) - 1][0..n].iter().copied().min().unwrap();
