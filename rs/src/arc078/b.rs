...
fn calc_size(g: &Graph, v: usize, p: usize, sizes: &mut [usize]) -> usize {
    sizes[v] = 1 + g.out_edges[v]
        .citer()
        .filter(|&e| e.to != p)
        .map(|e| calc_size(g, e.to, v, sizes))
        .sum::<usize>();
...
fn find_path(g: &Graph, v: usize, p: usize, dst: usize) -> Option<Vec<usize>> {
    if v == dst {
        ...
        g.out_edges[v]
            .citer()
            .filter(|&e| e.to != p)
            .find_map(|e| find_path(g, e.to, v, dst))
...
fn main() {
    ...
    calc_size(&g, 0, n, &mut sizes);
    let path = find_path(&g, 0, n, n - 1).unwrap();

    let idx = path
        .citer()
        .rev()
        .rposition(|v| sizes[v] >= n - sizes[v])
        .unwrap();

    if idx <= path.len() - 1 - idx {
        println!("Fennec");
    } else {
        println!("Snuke");
    }
