...
fn dfs(g: &Graph, v: usize, dst: usize, visited: &mut [bool]) -> bool {
    if visited[v] {
        return false;
    }
    if v == dst {
    ...
    g.adjs(v).any(|u| dfs(g, u, dst, visited))
}
...
fn main() {
    ...
    let g = Graph::from_edges_undirected(n + 1, lr.citer().map(|(l, r)| (l - 1, r)));

    if dfs(&g, 0, n, &mut vec![false; n + 1]) {
        println!("Yes");
    } else {
        println!("No");
    }
