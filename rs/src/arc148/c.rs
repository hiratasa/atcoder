...
fn main() {
    ...
    let g0 = Graph::from_edges_undirected(
        n + 1,
        once((0, 1)).chain(p.citer().enumerate().map(|(i, pp)| (i + 2, pp))),
    );
    let g = Graph::from_edges_directed(
        n + 1,
        once((0, 1))
            .chain(p.citer().enumerate().map(|(i, pp)| (i + 2, pp)))
            .map(|(u, v)| {
                if g0.deg(u) < g0.deg(v) {
                    (u, v)
                } else {
                    (v, u)
                }
    ...
    let query = read_vec(q, || {
        let query = read_row::<usize>();

        query[1..].citer().collect::<Vec<_>>()
    });

    query
        .iter()
        .map(|vs| {
            let set = vs.citer().collect::<FxHashSet<_>>();
            vs.citer()
                .sorted_by_key(|&v| g0.deg(v))
                .scan(FxHashMap::default(), |in_counts, v| {
                    let c0 = g.adjs(v).filter(|&u| !set.contains(&u)).count();
                    g.adjs(v).filter(|&u| set.contains(&u)).for_each(|u| {
                        *in_counts.entry(u).or_insert(0) += 1;
                    });

                    Some(c0 + (g0.deg(v) - g.deg(v) - in_counts.get(&v).copied().unwrap_or(0)))
                })
                .sum::<usize>()
        })
        .for_each(|ans| {
