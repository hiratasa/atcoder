...
fn dfs(
    ...
    prevs: &mut [Option<usize>],
    nexts: &mut [Option<usize>],
) {
    depth[v] = d;
    parents[v] = Some(p).filter(|&p| p < g.size());
    g.children(v, p).tuple_windows().for_each(|(prev, next)| {
        prevs[next] = Some(prev);
        nexts[prev] = Some(next);
    });
    g.children(v, p)
        .for_each(|u| dfs(g, u, v, d + 1, depth, parents, prevs, nexts));
}

fn main() {
    let n = read::<usize>();
    let abt = read_vec(n - 1, || read_tuple!(usize, usize, usize));

    let tree = UnweightedGraph::from_edges1_undirected(
        n,
        abt.citer()
            .filter(|&(_, _, t)| t == 2)
            .map(|(a, b, _)| (a, b)),
    ...
    let g = WeightedGraph::from_edges_directed(
        5 * n,
        (0..n)
            .map(|i| (i + n, i + 4 * n, 0))
            .chain(
                abt.citer()
                    .filter(|&(_a, _b, t)| t == 1)
                    .flat_map(|(a, b, _)| {
                        it![(a - 1 + 4 * n, b - 1, 1), (a - 1 + 4 * n, b - 1 + n, 1)]
                    }),
            )
            .chain(
                abt.citer()
                    ...
                    .flat_map(|(a, b)| it![(a, b, 1), (b + n, a + n, 1), (a, b + 4 * n, 1),]),
            )
            .chain((0..n).map(|i| (i, i + 4 * n, 0)))
            ...
            .chain((0..n).filter_map(|i| {
                let prev = prevs[i]?;
                Some((i + 3 * n, prev + 3 * n, 0))
            }))
            .chain((0..n).map(|i| (i + 2 * n, i + n, 0)))
            .chain((0..n).map(|i| (i + 3 * n, i + n, 0))),
    );

    let mut in_degs = (0..5 * n).map(|i| g.in_edges[i].len()).collect::<Vec<_>>();
    let mut zeros = in_degs.citer().positions(|d| d == 0).collect::<Vec<_>>();
    ...
    assert!(sorted.len() == 5 * n);

    let dp = sorted.citer().rev().fold(vec![0; 5 * n], |mut dp, v| {
        dp[v] = g.out_edges[v]
            .citer()
            .map(|e| dp[e.to] + e.label)
            .max()
            .unwrap_or(0);
        dp
    });
    let ans = dp.citer().max().unwrap() - 1;
