...
fn main() {
    let (n, _q) = read_tuple!(usize, usize);
    ...
    while m > 1 {
        if m == 2 {
            let u = g.adjs(v).filter(|&u| !used[u]).next().unwrap();

            println!("? {} {}", v + 1, u + 1);
            let reply = read::<usize>();
            assert!(reply == v + 1 || reply == u + 1);

            v = reply - 1;
            break;
        }

        let c = find_centroid(&g, v, n, &used, &mut sizes, m).unwrap();

        let get_size = |v: usize| {
            if sizes[v] > sizes[c] {
                m - sizes[c]
            } else {
                sizes[v]
