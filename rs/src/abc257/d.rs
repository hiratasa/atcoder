...
fn dfs(g: &Graph, v: usize, visited: &mut [bool]) -> usize {
    ...
    1 + g.adjs(v).map(|u| dfs(g, u, visited)).sum::<usize>()
}
...
fn main() {
    ...
    let ans = lower_bound_int(0, 1i64 << 40, |s| {
        let g = Graph::from_edges_directed(
            n,
            iproduct!(0..n, 0..n).filter(|&(i, j)| {
                s.saturating_mul(xyp[i].2)
                    >= (xyp[i].0 - xyp[j].0).abs() + (xyp[i].1 - xyp[j].1).abs()
            }),
        );

        if (0..n).any(|i| dfs(&g, i, &mut vec![false; n]) == n) {
            Ordering::Greater
        } else {
            Ordering::Less
        }
