...
fn dfs(g: &Graph, v: usize, p: usize, limit: usize) -> (usize, usize) {
    g.children(v, p)
        .map(|u| dfs(g, u, v, limit))
        .map(|(d, k)| (d + 1, k))
        .sorted_by_key(|&(d, _)| Reverse(d))
        .fold((0, 0), |(md, sk), (d, k)| {
            if d > limit {
                (md, sk + k + 1)
            } else if d + md > limit {
...
#[allow(dead_code)]
fn solve0(g: &Graph, k: usize) -> usize {
    let n = g.size();
...
fn main() {
    ...
    let ans = lower_bound_int(1, n, |t| {
        let (_, l) = dfs(&g, 0, n, 2 * t);
