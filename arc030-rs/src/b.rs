...
fn dfs(g: &UndirectedGraph, h: &Vec<usize>, v: usize, p: usize) -> Option<usize> {
    g.adjs[v]
        .iter()
        .copied()
        .filter(|&u| u != p)
        .map(|u| dfs(g, h, u, v))
        .fold(if h[v] > 0 { Some(0) } else { None }, |r, x| {
            x.map(|x| x + 2 + r.unwrap_or(0)).or(r)
        })
}
...
fn main() {
    ...
    let ans = dfs(&g, &h, x - 1, n).unwrap_or(0);
    println!("{}", ans);
}
