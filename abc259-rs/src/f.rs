...
fn dfs(g: &Graph, v: usize, p: usize, d: &[usize]) -> (i64, i64) {
    g.children_edge(v, p)
        .map(|e| (dfs(g, e.to, v, d), e.label))
        .map(|((s0, s1), w)| (s0, s1 + w))
        .map(|(s0, s1)| (max(0, s1 - s0), s0))
        .sorted_by_key(|&(x, _)| Reverse(x))
        .enumerate()
        .fold(
            (0, if d[v] == 0 { -(1 << 60) } else { 0 }),
            |(s0, s1), (i, (x, y))| {
                if i + 1 < d[v] {
                    (s0 + x + y, s1 + x + y)
                } else if i < d[v] {
