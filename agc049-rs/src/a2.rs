...
// 強連結成分分解
#[allow(dead_code)]
fn scc(g: &Graph) -> (Vec<Vec<usize>>, Vec<usize>, Vec<usize>) {
    let mut vs = vec![];
    {
...
fn solve(adjs: &Vec<Vec<usize>>, v: usize, sizes: &Vec<usize>, mut ex: f64, mut s: usize) -> f64 {
    s += sizes[v];
    ex += sizes[v] as f64 / s as f64;
    for &u in &adjs[v] {
        assert!(u != v);
        ex = solve(adjs, u, sizes, ex, s);
    }
...
fn main() {
    ...
    let (scc_graph, sizes, roots) = scc(&g);

    let mut ans = 0.0;
    for i in roots {
        ans += solve(&scc_graph, i, &sizes, 0.0, 0);
    }
