...
fn check(g: &Graph, abc: &[(usize, usize, i64)], excluded: usize, edges: &mut Vec<usize>) -> usize {
    let n = g.size();
    let mut stack = vec![];
    let mut potential = vec![None; n];
    ...
    while let Some((v, p)) = stack.pop() {
        for e in g.out_edges[v].citer() {
            ...
            let w = if abc[e.label].0 == e.from + 1 {
                abc[e.label].2
            } else {
                -abc[e.label].2
            };
            if let Some(q) = potential[e.to] {
                if q != p + w {
                    violation += 1;
                }
            } else {
                stack.push((e.to, p + w));
                potential[e.to] = Some(p + w);
                edges.push(e.label);
            }
        }
    }
...
fn main() {
    ...
    let g =
        Graph::from_edges1_undirected(n, abc.citer().enumerate().map(|(id, (a, b, _))| (a, b, id)));

    let mut edges = vec![];
    // 行きと帰りの2本まで許容
    if check(&g, &abc, usize::MAX, &mut edges) <= 2 {
        println!("Yes");
        return;
    }
    ...
    for excluded in edges {
        let mut edges = vec![];
        if check(&g, &abc, excluded, &mut edges) == 0 {
            println!("Yes");
            return;
        }
    }
