...
fn main() {
    ...
    let mut costs = vec![vec![usize::MAX; n]; 1 << n];
    let mut q = BinaryHeap::new();
    for i in 0..n {
        costs[1 << i][i] = 1;
        q.push((Reverse(1), 1 << i, i));
    }

    while let Some((Reverse(cost), s, v)) = q.pop() {
        if cost > costs[s][v] {
            continue;
        }
        ...
        for u in g.adjs(v) {
            ...
            if cost + 1 < costs[t][u] {
                costs[t][u] = cost + 1;
                q.push((Reverse(cost + 1), t, u));
            }
        }
    }

    let ans = (1..1 << n)
        .map(|s| costs[s].citer().min().unwrap())
        .sum::<usize>();
    println!("{}", ans);
}
