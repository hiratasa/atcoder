...
fn main() {
    let (n, k, l) = read_tuple!(usize, usize, usize);
    let pq = read_vec(k, || read_tuple!(usize, usize));
    let rs = read_vec(l, || read_tuple!(usize, usize));

    let mut uf1 =
        pq.citer()
            .map(|(p, q)| (p - 1, q - 1))
            .fold(UnionFind::new(n), |mut uf, (p, q)| {
                uf.unite(p, q);
                uf
            });
    ...
    let ans = (0..n)
        .map(|i| ((uf1.root(i), uf2.root(i)), i))
        .into_group_map()
        .into_iter()
        .fold(vec![0; n], |mut ans, (_, v)| {
            let s = v.len();
