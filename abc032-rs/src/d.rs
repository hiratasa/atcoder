...
#[allow(unused_imports)]
use itertools::{chain, iproduct, izip, repeat_n, Itertools};
#[allow(unused_imports)]
...
fn main() {
    ...
    let ans = if n <= 30 {
        ...
        vw.iter().copied().fold(vec![0; cap + 1], |prev, (v, w)| {
            izip!(
                prev.iter().copied(),
                repeat_n(0, w).chain(prev.iter().copied().map(|vv| vv + v)),
            )
            .map(|(v1, v2)| v1.max(v2))
            .collect_vec()
        })[cap]
        ...
        vw.iter()
            ...
            .fold(
                ...
                |prev, (v, w)| {
                    izip!(
                        prev.iter().copied(),
                        repeat_n(w, v).chain(prev.iter().copied().map(|ww| ww.saturating_add(w))),
                    )
                    .map(|(v1, v2)| v1.min(v2))
                    .collect_vec()
                },
