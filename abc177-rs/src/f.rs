...
define_monoid!(Replace, Option<usize>, None, Option::or);

impl Operator<usize> for Replace {
    fn apply(op: &Self::Item, v: &usize) -> usize {
        op.unwrap_or(*v)
    }
}

type ST = LazySegmentTree<Minimum, Replace>;
    ...
    ab.citer()
        ...
        .scan(
            ...
            |(st, st_dec), (i, (a, b))| {
                ...
                if b + 1 < w {
                    let x = st_dec.query(a, b + 2) + (b + 1) - (w - 1);

                    st.set(b + 1, x);
                    st_dec.set(b + 1, x + w - 1 - (b + 1));
                }

                st.update(a, b + 1, Some(1 << 60));
                st_dec.update(a, b + 1, Some(1 << 60));

                Some(st.query(0, w) + i + 1)
            },
        )
        .take_while(|&x| x < (1 << 40))
        .map(Some)
