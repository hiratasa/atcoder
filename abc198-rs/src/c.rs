...
fn main() {
    let (r, x, y) = read_tuple!(usize, usize, usize);

    // let n = lower_bound_int(0, 100000, |n: usize| {
    //     (n * n)
    //         .saturating_mul(r * r)
    //         .cmp(&(x * x + y * y))
    ...
    let n = ((x as f64).hypot(y as f64) / r as f64).ceil() as usize;

    let ans = if n == 1 && x * x + y * y != r * r {
        2
    } else {
        n
    };
