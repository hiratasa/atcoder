...
fn all_extend_rects(
    ...
    let ans0 = param.params[idx..]
        .iter()
        .enumerate()
        .fold(prev_ans.clone(), |mut ans, (ii, p)| {
            assert!(ans[idx + ii].0 == p.i);
            ans[idx + ii].1 = Rectangle::new(p.x, p.y, p.x + 1, p.y + 1);
            ans
        });
    ...
    param.params[idx..]
        .iter()
        .enumerate()
        .try_fold((ans0, idx), |(mut ans, first_changed_idx), (ii, p)| {
            let ii = idx + ii;
                    ...
                    (0..n)
                        .map(|i| {
                            if i < idx {
                                ...
                                param.params[idx].clone()
                            } else {
                                param.params[i + 1].clone()
                            }
                        })
                        .collect()
                } else {
...
fn generate_operation<R: rand::Rng>(rng: &mut R, n: usize) -> Operation {
    ...
    if t < 0 {
        ...
        ShuffleOrder(dist_idx.sample(rng), rng.gen())
    } else if t < 80 {
    ...
    unfold(
        ...
        |(param, ans, score, best_score, i_trial)| {
            ...
            *i_trial += 1;
            ...
            // };
            } else if let ShuffleOrder(idx, b) = op {
                let reordered_ans = if b {
                    (0..n)
                        .map(|i| {
                            if i < idx {
                                ans[i].clone()
                ...
                all_extend_rects(n, &param, idx, &reordered_ans).unwrap_or(reordered_ans.clone())
