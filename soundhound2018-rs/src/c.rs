...
fn main() {
    ...
    let cells = read_vec(r, || read_str());

    let mut g = MaxFlowGraph::new(r * c + 2);

    let src = r * c;
    let dst = r * c + 1;
    iproduct!(0..r, 0..c).for_each(|(i, j)| {
        let id = i * c + j;
        ...
        if (i + j) % 2 == 0 {
            ...
            it![(usize::MAX, 0), (1, 0), (0, usize::MAX), (0, 1)]
                ...
                .filter(|&(ni, nj)| ni < cells.len() && nj < cells[0].len())
                .for_each(|(ni, nj)| {
                    let id2 = ni * c + nj;

                    g.add_edge(id, id2, usize::MAX);
                });
        } else {
            g.add_edge(id, dst, 1);
        }
    ...
    let s = cells
        .iter()
        .map(|row| row.citer().filter(|&x| x == '.').count())
        .sum::<usize>();
    let ans = s - f;
